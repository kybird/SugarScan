...
## Acceptance Criteria
<!-- kanban:ac:begin -->
- [x] #1 혼합 코퍼스 구축 — TC/TE 이미지를 tc_/te_ 접두로 복사해 한 디렉토리로, 곡선 장 목록 병합 검증, 기존 TC/TE/VE 코퍼스 불변
- [x] #2 8시드(디스크·시간 여유에 따라 4) 학습 + VE 3분위·로보플로우 dev 586 채점 — atone/tone2te 와 같은 자
- [x] #3 사전등록 판정 기록 — VE 작음 97% 이상 유지 && 로보플로우 통과 차이의 95% CI 가 0 을 포함하거나 양 → 채택 후보. VE 만 충족 → '구도 축 단독으로 실사진 회귀를 못 막는다'로 기록
- [x] #4 자(구축·판정 스크립트)와 BAND_EXP_PLAN 신규 절 커밋
<!-- kanban:ac:end -->
...
## Result
- 2026-09-23T09:42-07:00 — 사전등록 판정: VE만 충족 — tmix 작음 97.40%(≥97 통과, atone 대비 +4.86 CI 양) 그러나 로보플로우 dev 586 통과 90.87(Δ−2.56 [−4.44,−0.81] 유의 하락, 조건② 기각). TE 단독(−4.94) 대비 회귀 절반 회복. 결론: 구도 축 혼합만으로 실사진 회귀를 못 막는다 — 다음 축은 질감·장면(실사진닮음). 자: mix_corpus.py·run_mix_arms.sh(신규) · tone2te_report.py --arms 3팔 확장. 기록: §32 · raw Case 9.
