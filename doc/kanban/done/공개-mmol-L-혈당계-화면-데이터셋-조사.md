...
- [ ] #4 7Seg 데이터셋(7-세그먼트 인식 벤치마크)의 공개 여부·라이선스·장수도 같은 표에 조사한다
<!-- kanban:ac:end -->
...
- 2026-09-11T21:02-07:00 — 후보 추가(2026-09-11): 7Seg 데이터셋 — 문헌에서 7-세그먼트 인식 벤치마크로 쓰인다(PARSeq 가 여기서 56.97%, DBNet 이 검출 최고로 보고됨). 혈당계 전용은 아니지만 7-세그 실사진이라 우리 합성의 현실성 검증과 사전학습에 쓸 수 있다. 논문: Detecting and recognizing seven segment digits using a deep learning approach (ITM Conferences 2024, itmconf_amict2023_01007). 다운로드 링크가 공개돼 있는지, 라이선스가 무엇인지가 조사 대상이다 — 없으면 '공개 링크 미확인' 으로 적는다.
