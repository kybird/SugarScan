...
- 지금은 앱 버전이 하나뿐이라 발생률 0.
- **급하지 않다(2026-09-02 이후).** 1단계(모르면 던지고 행을 건너뛴다)가 이미
  파괴를 막았으므로 되돌릴 수 없는 상태가 아니다. 이 패턴은 "구버전에서도 값과
  시각은 보이게" 하는 UX 개선이지 데이터 보호가 아니다. 필요해지면 그때 올린다.
