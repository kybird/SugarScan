...
**어떤 자로 (2) — 자가 이미 있는데 쓰지 않은 경우.** 엣지 밀도를 즉석
스크립트로 재 7.51% 를 얻고, GLM 이 커밋된 자로 잰 2.25% 와 나란히 놓았다.
자(`measure_panel_stats.py`)는 저장소에 있었고 손잡이까지 코드에 적혀 있었다.
손잡이를 전부 돌려도 7.51% 는 나오지 않아 **출처 불명으로 폐기**했다 —
...
- **재기 전에 같은 축을 재는 자가 저장소에 있는지 찾는다.** 있으면 그걸 쓰고,
  없어서 만들었으면 그 스크립트를 커밋한다 — 커밋되지 않은 스크립트로 잰 값은
  인용할 수 없다
- 수치 옆에 **그 값을 내는 명령과 `n`** 을 적는다. 손잡이는 기본값이라도 적는다
- 두 값이 갈리면 해명하지 말고 **상대의 자를 읽고 돌린다**
