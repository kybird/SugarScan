...
- **메인 작업대(`/`) 머리글의 화면 링크 줄(`nav.pages`)이 이 표의 거울이다** —
  화면을 늘리거나 줄이면 둘을 함께 고친다(2026-09-21 추가. 이전엔 메인에
  링크가 하나도 없어 URL 을 직접 쳐야 했고, 그래서 늘어난 화면이
  발견되지 않았다 — 사람 지적: "메인에서 이동할수있게 엔트리를 넣어라").
- **상단 메뉴는 모든 페이지가 공유한다**(`webtool.py` 의 `_topnav` 한 곳에
  정의, `_send_page` 가 전 페이지에 주입 — 2026-09-21 사람 지시: "상위메뉴는
  모든 페이지가 공유하게 하자"). 독립 화면 7개는 주입받고, 작업대(`/`)는
  자체 탭+버튼이 같은 역할을 한다(`/#탭` 해시로 어느 페이지서든 작업대의
  특정 탭으로 바로 간다). **화면을 늘리거나 줄이면 `_NAV_PAGES` 도 함께
  고친다.**
...
`arms_view.html` 은 지웠다. `/api/arms` 는 집약 모드가 읽는 데이터원으로
남는다. 비교·검사 질문은 이제 **한 화면의 세 모드**다: 단장(시드 하나,
방금 추론) · 목록(전량 라이브 스캔) · 집약(4시드, 시드 잡음까지 재는 배치).
...
1. ~~**`/arms` 와 `/livecmp`** 겹침~~ → **2026-09-21 흡수 완료.** 비교·검사는
   `/livecmp` 의 세 모드(단장·목록·집약)로 통합됐다. **새 비교 화면은
   만들지 않는다 — `/livecmp`를 늘린다.**
