...
| `/livecmp` | live_view.html | 라이브 팔 비교 — 요청 때마다 추론, 겹침/나란히 | 체크포인트 **라이브 추론** + 사람 라벨 | 09-21 |
...
1. **`/arms` 와 `/livecmp`** — 같은 질문(두 팔 눈비교)을 다른 방식.
   ...
   `/livecmp` = 라이브 단장(시드 개별, 즉석). 당장 병립 근거는 있으나
...
3. **훈련 탭·기종 탭의 `band_det_*` 갈아끼우기** — 전부 폐기된 옛 계열
   ...
   드롭다운 정리 후보.
4. **`/bandreal`** — `_diag` 배치 읽기로 `/livecmp`와 부분 겹침. 시드
   전체 집약 기능이 있어 당분간 병립, 장기 흡수 후보.
