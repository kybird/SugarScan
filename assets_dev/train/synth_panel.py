...
# 2026-09-20 사람 선언: "합성기 수정해.. 모든문제는 합성기수정으로 잡힌다.
# 그리고 혈당기위치를 좀더 가장자리에 있는 이미지도 많이 추가하자"
#
# 하한을 0.25 -> 0.10 으로 내린다. 0.25 로는 **실촬 크기에 닿지 않는다**:
#   밴드 높이 / 긴 변   합성 중앙 0.484 · 실촬 중앙 0.118 · 실촬 p10 0.081
#   배율 0.25 에서 합성이 닿는 최소가 약 0.195 라 실촬 중앙에도 못 미친다.
# 근거는 diag_center_gap.py · BAND_EXP_PLAN §19.3·§21.
#
# **증강으로 줄이는 것과 다르다.** 증강은 이미 그려진 896 그림을 또 줄여
...
#
# 왜: _zoom_mat 이 캔버스 중심 기준으로 축소해서, 배율을 낮춰도 기기는 늘
# 한가운데 앉았다. 실촬은 그렇지 않다 — 밴드 중심의 화면중앙 거리가
# 학습 코퍼스 중앙 0.058 대 실촬 0.105 이고, 세로 산포는 0.031 대 0.091 로
# 세 배다(diag_center_gap.py).
#
# 여백 비율로 정의하므로 **상자가 화면 밖으로 나가지 않는다** — 1.0 미만이면
# 피사체는 항상 캔버스 안이다.
CAM_OFFCENTER = 0.75           # 2026-09-20 사람 선언
...
def _render_once(value, rng, profile, pid, inverted, scene="panel"):
    ...
    _prng = random.Random(_oseed ^ 0x7F4A7C15)
    _off_fx = _prng.uniform(-CAM_OFFCENTER, CAM_OFFCENTER)
    _off_fy = _prng.uniform(-CAM_OFFCENTER, CAM_OFFCENTER)
    ...
    def _zoom_mat(z):
        """축소하고 **화면 안 임의의 자리에** 앉힌다.

        구판은 캔버스 중심 고정이라 물러나 찍어도 기기가 늘 한가운데였다.
        밀어내는 폭은 **남는 여백의 비율**로 정하므로(CAM_OFFCENTER<1)
        피사체가 캔버스를 벗어나지 않는다. 사다리가 배율을 더 낮추면
        여백이 커지고 밀 수 있는 폭도 같이 커진다 — 비율로 정의한 덕이다.
        """
        c = np.float32([[W / 2.0, H / 2.0]])
        dst = (src - c) * z + c
        if CAM_OFFCENTER > 0.0 and z < 0.999:
            dst = dst + np.float32([[_off_fx * (1.0 - z) * W / 2.0,
                                     _off_fy * (1.0 - z) * H / 2.0]])
