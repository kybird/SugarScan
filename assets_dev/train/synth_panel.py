...
#
# **그래서 이 값은 지금 틀렸다. 그러나 내가 고쳐 넣지 않는다.**
# antipatterns/no-real-photo-ruler-for-synth 가 금지한다 — 합성 설계값을
# 실사진 측정으로 되먹이면 안 되고, 사람이 보고 선언해야 한다. 위 표는
# 선언에 필요한 재료이지 자동 조정의 입력이 아니다.
...
def _render_once(value, rng, profile, pid, inverted):
    ...
    # 0.10 에서도 13% 가 이미 반듯해져 있었다 — cam_roll ±15° 를 넓힌 효과가
    # 그만큼 깎이고 있었다.
    #
