...
#   출력   특징맵 한 장(단일 스케일). /16 이 기본이고 /8 도 고를 수 있다.
#          ~~stride 8 을 만들지 않는다 — 실측으로 밴드 짧은변이 입력 416 에서
#          실촬 중앙 87px 라 stride 16 에서도 5.4칸을 덮는다~~ → 2026-09-21
#          하루 허용했다가(SPEC §5.3) **8번 실험이 기각해 되돌렸다**
#          (BAND_EXP_PLAN §25): 같은 52x52 격자를 416+/8 로 얻어도 노린 층이
#          전혀 안 움직였다 — 병목은 격자가 아니라 입력 화소다. stride 8
#          코드는 기각 재현을 위해 남긴다. stride 32 는 성기다 — 안 쓴다.
#          다중 스케일 FPN 은 계속 제외: 헤드는 언제나 한 장이다.
