...
OUT = HERE / "datumo_quads_v2.jsonl"
CONF = 0.25
...
def main() -> int:
    ...
    model = exp.get_model()
    ckpt = torch.load(HERE / "yolox_out" / "reading_v2" / "latest_ckpt.pth", map_location="cpu")
    ...
    trained = set()
    for line in (HERE / "labeled.jsonl").read_text(encoding="utf-8").splitlines():
        if not line.strip():
            continue
        j = json.loads(line)
        if j.get("source") == "human":
            trained.add(j["id"])
    print("hold-out 제외(학습에 쓴 장):", len(trained))
    n_ok = 0
