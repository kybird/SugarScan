...
# 절단형 렌더의 변형별 trim(2026-09-25 사람 지적 "획간 거리가 과도하게
# 멀다 — 얇을수록 가깝고 두꺼울수록 멀게" 실측 조정): 갭을 DSEG 원본과
# 맞춘 최소 안전값 — Light .11→3.7px(원본 2.7) · Regular .20→2.3(3.7) ·
# Bold .32→3.7(4.1) · Italic .23→2.7(3.7) · LightItalic .11→2.7(2.7) ·
# BoldItalic .32→3.7(3.7). 절단 돌출(t·tan15.1°≈0.27t)이 굵기 비례라
# 두꺼운 변형이 자동으로 넓다 — OLD 의 경향과 동일.
...
GLYPH_TRIM = {"Light": 0.11, "Regular": 0.20, "Bold": 0.36,
              "Italic": 0.26, "LightItalic": 0.14, "BoldItalic": 0.36}
    ...
    for i in range(1, n0):
        ...
        if not horiz:
            ...
            if is_top:
                ...
                cy = (y0 + y1) / 2.0 + pd + (trim * t - mid_extra) / 2.0
            else:
                L = max(1.0, float(hgt) - trim * t + mid_extra)
                cy = (y0 + y1) / 2.0 + pd - (trim * t - mid_extra) / 2.0
        else:
...
# 자를 만큼 붙은 기기가 있다. 생략하면 0.22 지만, **모든 프로파일이 0.05 를
# 명시한다**(사람 일괄 지시 2026-09-15) — 실물을 하나씩 보니 어느 기기도
# 기본값만큼 떨어져 있지 않았다.
#   hug=0.02 거의 닿는다  ·  0.10 이 밴드 경계  ·  0.45 여유
PROFILES = [
    dict(
    ...
    dict(
        ...
        # 하단 한 줄에 am/pm · 날짜 · 시각 순으로 왼쪽에 붙는다
        # (사람 눈검 2026-09-15, 1781·2498).
        time=dict(pos="row-bottom", order=["ampm", "date", "time"],
                  row_align="left", date_fmt="{M}-{d}", hm_fmt="{h02}:{m02}",
                  ampm=0.5, p=0.7),
    ...
    dict(
        ...
        # ── above-right 는 **대역 배치**다. 이 기기의 성질이 아니다 ──────────
        # 단위가 숫자 위인 실물은 **SD CodeFree** 다(사람 눈검 2026-09-16).
        # 그런데 SD CodeFree 는 프로파일이 없어서, 그 배치를 합성이 한 번도
        # 보여 주지 않았다 — 14개 프로파일이 전부 below-* 였다. 검출기가
        # "숫자 아래 작은 글자 줄"을 밴드 위치의 단서로 쓰면 그 기기에서 틀린다.
        #
        # 그래서 상단이 비어 있고(점유 0.0%) 아래가 붐비던(unit·time 이
        # below-right 를 다투던) 이 프로파일을 대역으로 세웠다. 사람 지시:
        # "그냥 진행해라".
        #
...
# 해서 올렸다. 잰 값이 아니다.
#
# 패딩 하한(2026-09-15, 사람 지침 "상하좌우 전부 패딩주자"):
#   좌 0.05 · 우 0.05 · 상 0.07 · 하 0.07 (유리 대비)
# 실물 액정은 유리 가장자리까지 세그먼트를 붙이지 않는다 — 구동 배선과
...
REGIONS = {
    ...
    "gc_ms_one": dict(pad=(0.050, 0.050, 0.070, 0.070), rows=(0.223, 0.462, 0.195)),
    "acura_plus": dict(pad=(0.058, 0.058, 0.070, 0.070), rows=(0.066, 0.585, 0.254)),
    "caresens_n_premier": dict(pad=(0.055, 0.055, 0.070, 0.070), rows=(0.100, 0.471, 0.316)),
    "performa_silver": dict(pad=(0.071, 0.071, 0.070, 0.070), rows=(0.172, 0.400, 0.308)),
    "performa_nano": dict(pad=(0.113, 0.113, 0.070, 0.070), rows=(0.181, 0.395, 0.303)),
    "accuchek_active": dict(pad=(0.075, 0.075, 0.070, 0.070), rows=(0.190, 0.420, 0.270)),
    "gluneo_plus": dict(pad=(0.104, 0.104, 0.070, 0.070), rows=(0.128, 0.466, 0.286)),
