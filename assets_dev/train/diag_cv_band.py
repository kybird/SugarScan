...
DEFAULTS = dict(
    ...
    empty_left=0,    # 1이면 두 자리 숫자줄 왼쪽을 피치만큼 확장(라벨 관례상 기본 끔)
...
def find_band(gray, P=None):
    ...
    x0, y0, x1, y1 = best[2]
    if P["empty_left"]:
        # 라벨 관례 확인 전 가설이었던 "빈 선행 슬롯 보강". 관례는 "두 자리 값은
        # 딱 두 자리만 라벨링"이라 기본 끈다(1000 처럼 왼쪽 빈 공간까지 덮은
        # 라벨은 관례 예외로 치고 쫓지 않는다). 스위치는 A/B 측정용으로 남긴다.
        hs = [g[3] for g in best[1]]
        dg = [g for g in best[1] if g[3] >= 0.6 * max(hs)]
        if len(dg) == 2:
            cx = sorted(g[0] + g[2] / 2 for g in dg)
            pitch = cx[1] - cx[0]
            if pitch > 0.5 * max(hs):
                x0 -= pitch
...
def main():
    ...
    for cid, tb in sorted(band.items()):
        ...
        if not p.exists():
            continue
        gray = load_gray(p)
        x0, y0, x1, y1 = crop_for(gray, gm[cid], P["pad"], P["pad_x"])
