...
| 밴드(숫자줄) 라벨링으로 전환 | ~~**보류.** 근거가 "GM 검출이 병목"이었는데 검출률 99.4%·판독 96.5%로 성립하지 않는다. 가로 화면조차 2%라 밴드 검출기를 새로 만드는 비용이 편익(≤1.17pp)을 넘는다.~~ **정정 2026-09-12 — 보류 사유 세 개가 전부 무너졌다. 재개 조건이다.** ①"검출률 99.4%"는 정확도가 아니라 **출력 존재율**이었다(`detect_datumo_gm.py` 는 박스가 나오면 `n_ok` 를 센다). 사람 LCD 라벨 393장 대비 IoU 중앙 0.795, **≥0.9 가 7.4%** — 거의 안 놓치지만 거의 안 맞는다. ②"≤1.17pp"는 **가로형 검출 실패**를 고쳤을 때의 기여이지 밴드 검출기의 편익이 아니다. 밴드의 편익은 전 기기에서 크롭이 조여져 단위·시간줄·아이콘이 프레임에서 빠지는 것이고, 기기 단절 홀드아웃 오독의 주된 축이 정확히 그 글리프다(Gmate·Instant). ③심증을 굳힌 G30/G31 밴드 크롭 A/B 는 **합성이 화면 전체 그대로**였다(G31 보고서 `[OK] 합성 항등: 500/500 픽셀 완전일치`) — 밴드 팔만 사전학습·파인튜닝 도메인 갭이 더 컸다. 그 교란은 보고서에 없다. | **재개** — 단 합성을 물리 패널 기준으로 고친 뒤. 순서는 `doc/kanban` 참조 |
...
## GM 검출 ft3 정본 승격 (2026-09-12)

`DONE.md` 표에 "실제 개선이지만 **운영 반영 여부 미정**"으로 1주일 넘게 멈춰
있던 것을 반영했다. 파이프라인이 base 검출 출력을 쓰고 있었고, 더 나은
출력 두 개(`_ft3`·`_ft4`)가 옆에 놓인 채 아무도 안 쓰고 있었다.

**판정 근거** — 사람 LCD 라벨과의 IoU 대조:

| 출력 | IoU 중앙 | ≥0.9 | ≥0.75 | <0.3 |
|---|---:|---:|---:|---:|
| base (쓰고 있던 것) | 0.795 | 7.4% | 65.6% | 0.5% |
| **ft3 (승격)** | **0.828** | **10.4%** | **76.3%** | **0.0%** |
| ft4 | 0.761 | 5.0% | 55.7% | 1.3% |

ft4 는 전 지표 퇴보라 쓰지 않는다 — 그동안 반영 안 한 것이 결과적으로 맞았다.

> 이 대조 표본(393장)은 두 모델의 학습 데이터와 겹친다(ft3 가 116장 더 봤다).
> **검출기 성능 수치로 인용하지 말 것.** 셋 중 무엇을 쓸지 고르는 데는 충분하다.

**섞지 않은 것**: 사람 LCD 라벨로 대체하지 않았다. 위 표 "가로 LCD 를 유리
전체로 라벨링" 항목이 이미 역효과로 기록돼 있다(사람 박스 9.7% vs GM 박스
51.6%) — 리더가 GM 박스로 학습돼 사람 박스가 분포 밖이다. 검출기 출력만 쓴다.

**한 갈래로 정리**: `gmscreen_quads.jsonl`(정본) → `convert_quads_oriented.py`
→ `gmscreen_quads_oriented.jsonl`(라벨러 힌트). 임시로 두었던 `_ft3_oriented`
파일은 지웠다. 이전 정본은 `gmscreen_quads_base_before_ft3_20260912.jsonl`.

### 캐시 재생성 전에 사람이 해야 할 일 — 지금은 멈춰 있다

승격은 파일 교체까지다. `data_cache_v2.npz` 를 다시 만들면 **풀이 바뀐다**:

- **새로 들어오는 8장** — `126` · `92` · `97` · `233` · `235` · `713` · `722` ·
  `2317`. base 가 못 찾아 학습에서 통째로 빠져 있던 장들이고, **8장 모두 사람이
  밴드를 그려 둔 장**이다(7장은 LCD 라벨도 있다). 앞서 "밴드 라벨 중 풀 밖
  18장"의 정체가 이것이다.
  **막는 것**: 이 8장에 기기 라벨이 없다(0/8). `build_device_split.py` 가
  "라벨이 풀을 덮지 않는다"로 멈춘다 — `device_labels.jsonl` 은 읽기 전용이라
  사람이 8장을 찍어 줘야 한다.
- **빠지는 5장** — `1046`(GT 142) · `1826`(157) · `2198`(115) · `498`(195) ·
  `702`(102). ft3 가 아무것도 못 낸 장들이다. 4장은 사람 LCD 라벨이 있다.
  **ft3 의 회귀**이므로 원인을 봐야 한다. 5장/2,494 = 0.2%.

그때까지 `data_cache_v2.npz` 와 `train_device/` 캐시는 옛 풀(2,494) 그대로다 —
`build_device_split.py` 는 캐시를 읽지 쿼드를 읽지 않으므로 지금 당장 깨지는
것은 없다.
...
## G21 — 안티패턴 체크리스트로 lib/ 전수 점검 (2026-09-10, 보고만)

8종 안티패턴을 `lib/` 전체에 적용, 코드 0줄 변경.
→ [`G21-antipattern-sweep.md`](reports/G21-antipattern-sweep.md)

**신규 3건** (이 보고서가 다음 사이클 작업 목록의 입력이다):
1. `_cropRoi` 세 벌(규칙 엔진·CNN 엔진·photo_align 인라인) — 현재 동치지만
   고정 테스트 없음, 한쪽 고치면 조용히 갈라짐(함정만).
2. 엔진 `recognize` 두 구현 무방비 — 계약 문구는 있으나 실제 방어는 스캐너의
   `_recognizeSafely` 한 곳(함정만).
3. 대시보드 `_openScanner` 저장 무가드 — 주장 문구("실패하지 않는다") 대비
   try 전무, 로컬 DB 실패 시 미처리 예외(조건부).

나머지 5종은 0건 — 알려진 사이트 3곳은 수정본 확인, 후보 전부 재검토해
해당 없음.

**다시 나지 않게 하는 것**: "터졌던 자리만 고치고 같은 모양이 다른 곳에 있는지
안 보는 것" — 이 스윕 자체가 그 처방이다. 다음에 안티패턴 페이지를 만들면
같은 날 스윕을 붙인다.

## G12 — l10n 정합성을 테스트로 고정 (2026-09-10)

`test/l10n/arb_consistency_test.dart` — 5검사(파일 6개·키 집합·@메타·플레이스
홀더·미참조). 파일 직접 읽기, 새 패키지 0. → [`G12-l10n-consistency-test.md`](reports/G12-l10n-consistency-test.md)

현재 상태 전부 깨끗(키 104씩 일치·미참조 0건·예외 목록 불필요). 고의 붕괴
검증: `app_de.arb` 에서 키 하나 제거 시 "app_de.arb 에 없는 키: [...]" 로
파일·키 이름을 찍어 실패 — 복원 확인 완료.

**다시 나지 않게 하는 것**: 키 집합·플레이스홀더 불일치를 리뷰 눈이 아니라
커밋 게이트로 잡는다. `@@locale` 은 ARB 규격의 로케일 선언이라 번역본 허용으로
읽었다(키 메타데이터만 원본 단독).

## G14 — 테스트 없는 화면 넷 채우기 (2026-09-10)

설정·수동입력·확인시트·동기배너에 위젯 테스트 15건 신규. 화면 코드 0줄 변경.
→ [`G14-untested-screens.md`](reports/G14-untested-screens.md)

확인 시트의 **"저장은 사용자 1탭"**(값을 고쳐도 취소하면 결과 없음)과 수동
입력의 **쉼표 소수점(7,6→7.6)**·범위 밖 거절, 서버 미설정 빌드의 동기화
영역/배너 숨김(배너는 실제 공급자 사슬로)을 고정했다. 전체 402 통과.

부수 발견(보고만): 수동 입력에서 네 자리 숫자는 범위 밖 문구가 아니라
"숫자를 입력하세요"로 거절된다 — 모양 검사(1~3자리)가 범위 검사보다
앞선다. 안전엔 무관, 문구 정확도 문제.

**다시 나지 않게 하는 것**: 시트 저장 버튼 전 `ensureVisible` 는 이 앱의
리터럴한 규칙이다 — 새 시트 테스트를 복붙할 때 이 줄이 빠지면 "저장 안 됨"
으로 잘못 통과한다(W12·이번 G14 하네스에 주석으로 명시).

## G18 — 셀 벤치 실패 표본 층화 (2026-09-10)

`--dump-failures N` 이 앞에서 자르는 바람에 전부 클래스 `00` 에서 소진되던
것을 클래스별 고르게 나눠 실도록 고쳤다. → [`G18-failure-stratification.md`](reports/G18-failure-stratification.md)

**진짜 병목은 2000건 상한이었다.** 층화만 넣은 첫 실행에서도 여전히 110건
전부 `00` — 이 데이터셋은 `00` 의 실패만 2000 을 넘겨 상한이 리스트를 `00`
으로 채웠고, 층화가 나눠 줄 대상 자체가 없었다. 상한 제거 후 11개 클래스 ×
10건 달성. 측정 통계는 상한 전후 동일(덤프 선택만 바뀐다는 증거).
...
## G19 — 셀 정규화를 플래그 뒤에 구현하고 A/B 측정 (2026-09-10, 채택 사람 대기)

`SegmentSampler.sample(normalizeToInk:)` — 잉크 경계 상자 기하(40%·95%·2%
규칙 지시서 그대로). 기본값 false, 부트스트랩 무수지.
→ [`G19-ink-normalization.md`](reports/G19-ink-normalization.md)

**A/B 결과 — 두 벤치 같은 방향으로, 값이 아니다.** 미판독이 값으로 바뀌며
정답·오독이 같이 오르고 오독 증가분이 상대적으로 크다(셀 +591건·장면 +46건).
새로 나온 값의 20.3%가 틀린 값. `0`→`H` 는 683→204 으로 G15 통증 자체는
크게 줄고, 표시 없음 화면의 값 생성은 4.32%→5.52% 로 악화. **기본값은 끔
유지 — 켤지는 사람이 정한다.**

**다시 나지 않게 하는 것**: "버리는 것을 값으로 바꾸는" 개선은 정답만 늘리는게
아니라 정답·오독을 같이 늘린다 — 채택 판정은 완전일치가 아니라 **치명적 오독
증가분과 새 값의 오독률**로 해야 한다.
