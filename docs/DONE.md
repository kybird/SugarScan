...
| **가로가 긴 LCD** (사용자 관찰, 2026-09-04) | **검출·판독 양쪽의 사각지대다.** GM 검출 중 w/h>1.5 는 1.24%(31/2494)뿐인데 사람 라벨은 11.9%(35/294) — GM 이 가로 화면을 못 찾거나 세로로 좁혀 잡는다(1091: 사람 3.50 → GM 1.38). 미검출 15장 중 4장이 극단 가로(3.94·3.44·2.53·2.13). **판독은 가로 51.6%(16/31) vs 세로 94.4%(234/248) — 42pp 격차.** 리더가 학습에서 가로 화면을 거의 못 봤다 | 열림 → 사람 판단 필요 |
| 가로 LCD 를 "유리 전체"로 라벨링하는 것 | **역효과.** 사람이 그린 전체 유리 박스로 읽으면 9.7%(3/31)로 GM 박스(51.6%)보다 나쁘다. 3.5:1 화면을 2:1 캔버스에 넣으면 숫자가 화면 일부만 차지해 작아진다. 단 리더가 GM 박스로 학습돼 사람 박스가 분포 밖이라는 교란이 있다 | 열림 — 리더가 원하는 건 유리가 아니라 **숫자 영역**일 수 있다 |
