...
| D-2 | 1차 OCR 엔진 | ~~EasyOCR~~ → 규칙 기반 판독기 → **자체 CTC 리더** | **결론 유지, 이유 정정(2026-09-05).** 원문은 "7-세그먼트는 상태 공간이 유한해 학습 모델이 과잉"이라고 적었으나 **그 이유는 틀렸다** — 숫자의 상태 공간은 유한하지만 **사진의 상태 공간은 무한하다**(조명·각도·반사·기종·폰트). 실제 이유는 **범용 OCR 이 7-세그먼트를 문자로 인식하지 못한다**는 것이고 이제 측정됐다: EasyOCR 우리 크롭 9.3% · 원본 전체 1.3% vs 자체 파이프라인 98.0%(표본 150). 획 간격을 글자 경계로 오인해 `177`→`2005`. 한편 그 이유를 믿고 채택한 규칙 판독기는 실촬 완전일치 **0%**(G17)라 자체 CTC 리더로 다시 바뀌었다. `doc/raw/2026-09-05.md#case-7` · `patterns/measure-the-premise-not-just-the-claim` |
...
**W8 잔여 과제** — 셋 다 해결됐다.
- ~~기록 편집 화면~~ — W12 에서 붙였다.
- ~~삭제 되돌리기~~ — W12. 되살리기도 아웃박스를 거친다.
- ~~표시 단위가 로케일 추정값~~ — W11-a 온보딩 확인.

### W11-a — 표시 단위 확인 온보딩

단위 오설정이 왜 "가장 위험한 UX 버그"인지 실제로 계산해 보고, 그에 맞춰
확인 게이트를 넣었다. 테스트 203개 통과.

**위험의 정확한 크기**

검증기 범위(mg/dL 10~900 정수, mmol/L 0.6~50 소수1자리)에서 **두 단위 모두를
통과하는 값은 10~50 사이의 정수 41개뿐**이다. 그 밖의 범위는 검증기가 걸러내
스캔이 실패할 뿐이라 사용자가 이상을 알아챈다. 문제는 이 구간이다.

| 같은 숫자 `40` | 의미 |
|---|---|
| mg/dL 로 읽으면 | **중증 저혈당** |
| mmol/L 로 읽으면 | 721 mg/dL 상당 — **중증 고혈당** |

단위가 반대로 잡히면 이 구간의 기록이 **임상적으로 정확히 반대 의미로** 저장된다.
하필 가장 중요한 값들이다. 검증기도 안정화기도 이 오류는 잡지 못한다 —
숫자 자체는 완벽히 정상이기 때문이다. 회귀 테스트로 이 구간을 고정해 두었다.

**대응**
- `UnitGate` 를 `MaterialApp.router` 의 `builder` 에 두어 **모든 경로**를 덮는다.
  대시보드에만 걸면 스캔 화면으로 직접 들어가는 경로가 뚫린다.
- 온보딩은 단위 이름이 아니라 **숫자 예시**(`138` / `7.6`)를 보여 준다. 사용자가
  자기 혈당계 화면과 눈으로 대조하는 편이 확실하다.
- 로케일 추정값은 미리 선택만 해 두고, 확정은 사용자의 탭으로만 이뤄진다.
  `UnitPreference.confirmedByUser` 가 "추정"과 "확인"을 구분한다.
- 설정 화면에서 언제든 바꿀 수 있다. 정본이 mg/dL 이라 표시만 바뀌고 저장된
  기록은 그대로다 — 각 기록은 입력 당시의 `enteredUnit` 을 간직한다.

**저장**: 스키마 v2, `app_settings` 키-값 테이블. 설정이 늘 때마다 스키마를
바꾸지 않기 위해서다. `onUpgrade` 마이그레이션 포함.

**남겨 뒀던 것**: 사용자가 단위를 잘못 고른 채 기록을 남겼다가 나중에 고치는
경우, 기존 행은 그대로다. 정본이 `enteredUnit` 기준으로 계산되어 있어 자동
정정은 위험하다 — 어느 쪽이 옳은지 앱이 알 수 없다. **W12 기록 편집 시트에서
사람이 직접 다시 지정하는 방식으로 풀었다.**

### W9-a — Supabase 연결 설정

서버 스키마·RLS·클라이언트 초기화·익명 로그인까지. 동기화 엔진(W10)은 아직
없고, 이 단계는 **보낼 곳을 만든 것**까지다. 테스트 242개 통과.

| 파일 | 역할 |
|---|---|
| `supabase/migrations/0001_init.sql` | 스키마 + RLS + `updated_at` 트리거 |
| `data/remote/supabase_config.dart` | `--dart-define` 주입, service_role 키 차단 |
| `data/remote/remote_backend.dart` | 초기화. 실패해도 던지지 않는다 |
| `data/remote/secure_session_storage.dart` | 세션을 Keystore/Keychain 에 |
| `data/remote/auth_repository.dart` | Google 로그인, 실패 분류 |
| `data/remote/google_sign_in_flow.dart` | ID 토큰 취득(플러그인 격리) |
| `data/remote/google_auth_config.dart` | 클라이언트 ID 주입 |
| `features/auth/auth_gate.dart` | 로그인 게이트 |
| `features/auth/sign_in_screen.dart` | 첫 실행 로그인 화면 |
| `data/remote/reading_dto.dart` | 서버 행 ↔ `GlucoseReading` |

**접속 정보를 저장소에 넣지 않는다.** publishable(anon) 키는 RLS 뒤에 있어
노출되어도 되는 값이지만, 코드에 박아 두면 프로젝트를 바꿀 때마다 코드를 고쳐야
하고 언젠가 같은 자리에 service_role 키가 들어간다. 주입 경로를 `--dart-define`
하나로 좁히고, `SupabaseConfig.parse` 가 service_role 키(새 형식 `sb_secret_`,
예전 JWT 형식 `"role":"service_role"`)를 **예외로 거부**한다. 이 키는 RLS 를
통째로 우회하므로 클라이언트에 들어가면 그 프로젝트의 모든 사용자 기록이 열린다.

**서버 설정이 없는 상태가 정상 상태다.** `initializeRemoteBackend()` 는 예외를
밖으로 내보내지 않고 `RemoteDisabled` / `RemoteFailed` / `RemoteReady` 중 하나를
돌려준다. Drift 가 정본이라 서버 없이도 스캔·저장·조회가 전부 동작해야 하는데,
초기화 실패로 앱이 뜨지 않으면 오프라인 우선 설계가 있으나 마나다. sealed 타입이라
호출부가 패턴 매칭으로 꺼내야만 클라이언트를 만질 수 있고, "없을 수도 있다"를
잊고 지나갈 수 없다.

**세션 토큰을 SharedPreferences 에 두지 않는다.** supabase_flutter 기본값이
그렇다. 루팅/탈옥 단말이나 기기 백업에서 평문으로 읽히는데, 이 토큰 하나면
그 사용자의 혈당 기록 전체가 열린다. `LocalStorage` 를 구현해 Keystore/Keychain
으로 옮겼다 — 비용이 클래스 하나뿐이라 안 할 이유가 없다. iOS 접근성은
`first_unlock`: 기본값 `unlocked` 는 재부팅 후 첫 잠금 해제 전까지 백그라운드
토큰 갱신을 막는다.

**서버 스키마는 로컬의 부분집합이다.** `ocr_raw_text`, `photo_path`,
`adjusted_by_user` 를 의도적으로 뺐다(수집 최소화, §10). 그래서 **pull 이
로컬 행을 통째로 덮어쓰면 안 된다** — `ReadingDto.fromJson` 이 돌려주는 그
값들은 "지워졌다"가 아니라 "서버가 모른다"는 뜻이고, 덮어쓰면 그 기기에만
있던 OCR 원문과 사진이 동기화 한 번에 사라진다. W10 이 지켜야 할 계약이다.

**`updated_at` 은 서버가 찍는다.** 클라이언트는 보내지 않고 트리거가 `now()` 로
덮어쓴다. 델타 pull 커서가 이 값 위에 서 있어서, 틀어진 단말 시계가 미래 시각을
심으면 그 뒤의 정상 변경들이 커서에 걸리지 않고 **통째로 유실된다.**
대신 LWW 비교가 서버 시계 기준이 되므로, W10 에서 클라이언트 기준 비교가
필요해지면 `client_updated_at` 열을 따로 추가한다(열 추가는 쉽고, 이미 올라간
잘못된 커서를 되돌리는 일은 어렵다).

**익명 로그인을 쓰지 않기로 했다(§7.2 계획 변경).** 계획서는 익명으로 시작해
나중에 Google/Apple 을 연결(link)하는 그림이었으나, 지원 대상을 Google 하나로
좁히면서 익명 단계가 값을 못 하게 됐다.

| | 익명 도입 | 로그인 전 = 완전 로컬 |
|---|---|---|
| 로그인 전 서버 백업 | 된다 | 안 된다 |
| `auth.users` 행 | 설치마다 1개, MAU 집계에 포함 | 없음 |
| 정리 작업 | 방치된 익명 계정 주기적 삭제 필요 | 없음 |
| 계정 승계 | identity 충돌 병합 로직 필요 | 불필요 |

로그인 전 상태를 **완전 로컬**로 두는 쪽을 택했다. 이게 성립하는 이유는 기록
id 를 클라이언트가 만들고 `user_id` 를 로컬 어디에도 저장하지 않기 때문이다 —
`ReadingDto.toJson(reading, userId)` 가 push 하는 순간에 찍는다. 그래서 로그인
전에 쌓인 아웃박스가 로그인 직후 그대로 올라간다. 잃는 것은 "끝까지 로그인하지
않는 사용자의 서버 백업" 하나인데, 그건 클라우드를 원하지 않겠다고 선택한
사용자다.

**로그인은 첫 실행 한 번만 요구한다.** 그런데 여기에 CLAUDE.md 의 "사용자가
기록을 남기지 못하는 상태를 만들지 않는다"와 충돌할 여지가 있다 — 리프레시
토큰이 만료·폐기되면 세션이 사라지고, 그때 다시 로그인 화면으로 막으면
**비행기 안에서 측정한 사람이 기록을 남길 방법이 없어진다.**

그래서 게이트가 막는 경우를 하나로 못박았다: **서버가 설정된 빌드 + 세션 없음
+ 이 기기에서 로그인한 적 없음.** 나머지는 전부 통과다. "로그인한 적 있음"은
세션과 별개로 `app_settings` 에 남긴다(`auth_signed_in_before`). 세션이 끊긴
기기는 로컬 모드로 계속 쓰고, 동기화만 세션이 돌아올 때까지 미뤄진다.
서버 미설정 빌드도 통과시킨다 — 로그인이라는 개념 자체가 없는 빌드를 막으면
앱이 벽돌이 된다. 이 판정 전부 `test/app/auth_gate_test.dart` 로 고정했다.

**ID 토큰 방식을 쓴다.** 브라우저를 띄우는 OAuth 리디렉트 대신 네이티브 계정
선택기에서 받은 ID 토큰을 `signInWithIdToken` 으로 넘긴다. 딥링크 왕복이 없고
앱을 벗어나지 않는다. 함정 하나: Android 가 넘기는 토큰의 `aud` 는 **웹
애플리케이션 유형 클라이언트 ID**(`serverClientId`)여야 한다. Android 유형 ID 를
넣으면 토큰 발급은 되는데 Supabase 가 거부해서 원인을 찾기 어렵다.

`AuthFailure` 를 `cancelled` / `configurationError` / `transient` 로 나눈 이유:
취소는 오류가 아니라 화면에 붉은 배너를 띄우지 않아야 하고, 설정 오류는 재시도해도
똑같이 실패하므로 일시적 오류와 섞으면 백오프만 돈다.

**실제 프로젝트**: `SugarScan` (ref `fzjlgsnwiqftlbcnzeui`), 리전 `ap-southeast-1`
(싱가포르). 계획서 D-3 은 GDPR 대응으로 EU (Frankfurt)를 지정했으나 사용자가
싱가포르를 선택했다. 혈당은 GDPR Art.9 특별범주라 EU 사용자를 받게 되면
싱가포르는 적정성 결정이 없어 SCC 등 이전 근거가 별도로 필요하다. 리전은
사후 변경이 불가능하고 프로젝트를 새로 만들어 옮겨야 한다. EU 출시를 검토할
때 다시 꺼내볼 것.

**W9 잔여 과제**
- Google Cloud 콘솔 쪽 설정(웹/Android/iOS 클라이언트 ID, SHA-1 등록)과
  Supabase Google 프로바이더 활성화. 코드는 준비됐고 값 주입만 남았다.
- Apple 로그인 — iOS 스토어 심사에서 요구될 수 있다. Google 만 지원하는 앱은
  App Store 심사 지침 4.8 에 걸릴 수 있으므로 iOS 출시 전에 재확인할 것.
- 계정 탈퇴 시 hard delete Edge Function
- 동기화 엔진 — W10. 아웃박스는 W8 부터 쌓이고 있고 소비자만 없다.

### W10 — 동기화 엔진

W8 부터 쌓이기만 하던 아웃박스에 소비자를 붙이고, 상태를 화면에 꺼냈다.
테스트 270개 통과.

| 파일 | 역할 |
|---|---|
| `data/sync/sync_engine.dart` | push → pull 한 회차 |
| `data/sync/outbox_repository.dart` | 큐 읽기·성공·실패 처리 |
| `data/sync/reading_api.dart` | 서버 표면(인터페이스 + Supabase 구현) |
| `data/sync/sync_cursor_store.dart` | 델타 pull 커서 |
| `data/sync/sync_scheduler.dart` | 언제 도는지 — 디바운스·중복 방지 |
| `app/sync_runner.dart` | 촉발 지점(시작·복귀·로그인·저장) |
| `features/sync/sync_status_banner.dart` | 막혔을 때만 뜨는 배너 |

**보내기가 받기보다 먼저다.** 반대로 하면 아직 안 보낸 로컬 변경 위에 서버의
옛 값이 덮이고, 그 다음 push 가 그 옛 값을 서버로 되돌려 보낸다. 사용자가 방금
고친 값이 조용히 원복되는 것이라 알아채기도 어렵다.

**LWW 를 `updated_at` 크기 비교로 하지 않았다(§7.2 계획 변경).** 서버
`updated_at` 은 서버 시계가, 로컬 `updatedAt` 은 단말 시계가 찍는다(W9-a 에서
트리거로 정한 결과다). 두 값을 크기 비교하면 시계가 틀어진 만큼 틀린다.
대신 **"보내지 않은 변경이 있으면 그것이 최신"** 으로 판정한다 — pull 은
`syncState == pending` 인 행을 건너뛰고, 그 변경은 다음 push 에서 올라가고,
그 다음 pull 이 서버가 정한 결과를 받아 온다. 승자를 정하는 것은 결국 서버이고,
그편이 클라이언트끼리 시계를 대조하는 것보다 정확하다.

**오프라인은 실패가 아니다.** 시도 횟수를 태우지 않는다. 태우면 지하철을 여섯
번 타는 것만으로 아웃박스가 영구히 막힌다. 그래서 `SyncOutcome` 에 `offline`
과 `failed` 를 따로 뒀고, 네트워크 확인이 push 보다 먼저 온다.

**보낼 것을 버리는 선택지는 없다.** 시도 한도(6회)에 닿으면 그 항목은 더
시도하지 않지만 **행은 남는다.** `blockedCount` 가 0 이 아니면 사용자에게
알려야 한다 — 조용히 안 올라가는 상태가 가장 나쁘다.

**커서는 `초과`가 아니라 `이상`으로 자른다.** 서버 `updated_at` 은 트랜잭션
시각이라 한 번에 올린 배치가 **전부 같은 값**을 갖는다. `초과`로 자르면 페이지
경계에 걸친 동일 시각 행들이 영영 안 넘어온다. 경계 행을 매번 다시 받는 비용이
훨씬 싸다 — 적용이 멱등이라 결과가 같다. 정렬도 `updated_at, id` 2단으로 고정해
페이지 경계에서 행이 새거나 겹치지 않게 했다.

**커서는 사용자에 딸린 값이다.** 계정이 바뀌었는데 이전 커서를 쓰면 새 사용자의
그 시점 이전 기록을 통째로 건너뛴다. 커서와 함께 소유자를 저장하고, 다르면
없는 것으로 취급한다. 로그아웃 시에도 지운다(`signOutProvider`).

**같은 기록의 여러 변경은 하나로 접는다.** 한 기록을 세 번 고치면 아웃박스에
세 줄이 쌓이지만 서버로 가는 것은 현재 상태 하나다. 중간 단계를 순서대로
재생할 이유가 없다.

**회차가 겹치지 않게 한다.** 두 회차가 동시에 돌면 같은 항목을 두 번 보내고,
뒤늦게 끝난 쪽이 이미 지워진 큐를 다시 지우려 든다. 실행 중 들어온 요청은
버리지 않고 "끝나면 한 번 더"로 접어 둔다.

**동기화 상태 표시**

`blocked` 를 UI 로 꺼내다가 실제 결함을 하나 찾았다. **한도에 걸린 항목은
`pending()` 이 영영 집어오지 않아서, "다시 시도" 버튼을 달아도 아무 일도
일어나지 않는다.** `OutboxRepository.retryBlocked()` 로 시도 횟수를 되돌리는
경로를 함께 만들었다. 되돌리기는 **사용자가 버튼을 누를 때만** 한다 — 자동으로
풀면 한도의 의미가 없어져 고쳐지지 않는 오류를 무한히 재시도한다.

판정은 `syncStatusProvider` 한곳에 모았다. 세 신호(대기 수, 막힌 수, 세션 유무)를
화면이 각자 조합하면 어떤 화면에서는 막힌 상태를 놓치는 조합이 생긴다.
우선순위는 **막힘 > 로그아웃 > 대기**다.

- **대시보드 배너는 막혔을 때만 나타난다.** "동기화됨" 배지를 상시 노출하면
  사용자가 읽지 않게 되고, 정작 막혔을 때 눈에 안 들어온다. 문구는 기록이
  사라지지 않았다는 사실부터 말한다 — 사용자가 걱정할 것은 데이터 유실이지
  업로드 실패가 아니고, 실제로 유실은 일어나지 않는다.
- **설정 화면은 평상시에도 숫자를 보여준다.** 상태를 확인하러 오는 곳이라
  아무것도 없으면 기능이 없는 것처럼 보인다.
- **서버 미설정 빌드에서는 통째로 숨긴다.** 있지도 않은 기능이 고장 난 것처럼
  보이게 하지 않는다.

**W10 잔여 과제**
- 지수 백오프가 아직 "다음 촉발까지 기다림"이다. 실패 간격을 attempts 기반으로
  벌리는 것은 주기 실행(WorkManager/BGTaskScheduler)과 함께 다뤄야 한다.
- ~~실기기 검증~~ — 2026-08-20 완료(Redmi Note 11 / Android 14). 구글 로그인 →
  `auth.users` 1행 생성(익명 0) → 직접 입력 저장 → **3.6초 뒤 서버 반영**.
  올라간 행에서 확인한 것: `value_mgdl` 정본과 `entered_*` 원본이 함께 남고,
  `tag` 가 `wireName` 으로 저장되며, `measured_at` 이 UTC 이고 `tz_name` +
  `utc_offset_minutes` 로 벽시계(23:35 KST)가 복원된다. 무엇보다
  **`updated_at`(14:35:14)이 `created_at`(14:35:10)보다 늦다** — 클라이언트가
  보낸 값이 아니라 서버 트리거가 찍었다는 증거다.
  재설치 후에도 로그인 화면이 뜨지 않아 Keystore 세션 복원도 확인됐다.
- 아직 안 해본 것: 다기기 pull, 충돌, 오프라인 후 복귀, 시도 한도 도달.

### W12 — 기록 편집과 삭제 되돌리기

W8 부터 열려 있던 구멍 둘을 닫았다. 저장소에 `update()` 는 있는데 화면이 없었고,
소프트 삭제에 되돌아올 길이 없었다. 테스트 287개 통과.

| 파일 | 역할 |
|---|---|
| `features/history/edit_reading_sheet.dart` | 값·단위·태그·메모 수정 |
| `data/repositories/glucose_repository.dart` | `restore()` 추가, 메모 지우기 |

**값을 표시 단위가 아니라 기록의 `enteredUnit` 으로 보여 준다.** 표시 단위로
보여 주면 mmol/L 로 넣은 7.6 이 mg/dL 사용자에게 137 로 보이고, 그걸 고쳐
저장하는 순간 원본이 사라진다. 왕복 오차를 막으려고 `enteredUnit`/`enteredValue`
를 따로 남겨 둔 의미가 없어진다. 위젯 테스트의 첫 항목이 이것이다.

**단위를 다시 지정할 수 있다.** W11-a 가 편집 UI 로 미뤄 둔 문제다. 앱이 어느
쪽이 옳은지 판정할 수 없으므로 사람이 정하되, 안전장치를 둘 붙였다.
- **반대 단위 환산값을 항상 표시한다.** 7.6 을 mg/dL 로 두면 "0.4 mmol/L 에
  해당"이 뜬다. 자기 혈당계를 아는 사람에게는 이 줄이 즉시 이상하게 보인다.
  앱이 옳고 그름을 말하지 않으면서 사용자가 판단할 근거만 주는 방식이다.
- 단위를 바꾸면 **"변환이 아니라 재해석"** 이라고 명시한다.

**측정 시각은 고칠 수 없다.** 태깅과 일별 집계가 `measuredAtLocalWallClock`
위에 서 있어서, 시각을 고치면 그 기록이 속한 날짜와 구간이 통째로 움직인다.
필요해지면 그때 따로 다룬다.

**되살리기도 아웃박스를 거친다.** 이미 서버로 삭제가 전파됐을 수 있고, 그렇다면
되살렸다는 사실도 똑같이 전파되어야 다른 기기에서 살아난다.

**도중에 잡은 결함 둘**
- **메모를 지울 수 없었다.** `update(note: null)` 이 "바꾸지 않음"이라, 시트가
  빈 메모를 null 로 돌려주면 예전 메모가 영원히 남는다. null 은 바꾸지 않음,
  빈 문자열은 지움으로 나눴다.
- **위젯 테스트가 잘못된 이유로 통과했다.** 시트가 기본 테스트 뷰포트보다 길어
  저장 버튼 탭이 화면 밖을 쳤다. "범위를 벗어난 값은 저장되지 않는다"가 검증기
  덕이 아니라 아무 일도 일어나지 않아서 통과하고 있었다. `ensureVisible` 로
  고쳤다 — 시트가 긴 화면에서는 이 함정을 항상 의심할 것.

**W12 잔여 과제**
- 실기기에서 수정 → 아웃박스 → 서버 반영 확인. push 는 W10 에서 확인했지만
  `update` 경로는 아직이다.
- OCR 기록을 나중에 고쳐도 `adjustedByUser` 는 그대로다. 그 플래그는 "확인
  시점에 인식값을 고쳤는지"를 뜻하므로 며칠 뒤의 편집과는 다른 사건이다.
  엔진 품질 지표로 계속 쓰려면 이 구분을 유지하는 편이 맞다.

### W11 — 통계 화면

마지막 placeholder 를 걷어냈다. 이제 모든 탭에 실제 화면이 있다. 테스트 309개 통과.

| 파일 | 역할 |
|---|---|
| `features/stats/stats_screen.dart` | 기간 선택·요약·목표범위·산점도·태그별 평균 |
| `app/providers.dart` | `statsWindowProvider`, `statsSummaryProvider` 등 |
| `data/repositories/glucose_repository.dart` | `watchBetween()` 추가 |

**기본 창은 14일**이다. eA1c 창과 맞췄다 — 두 숫자가 같은 화면에 나란히 서는데
서로 다른 기간을 보고 있으면 사용자가 둘을 대조하다 잘못된 결론에 이른다.

**판정하지 않기 위해 한 선택들**
- **점을 선으로 잇지 않는다.** 채혈 측정은 시점 표본이라 점 사이를 이으면 재지
  않은 구간에 없는 정보를 그려 넣는 셈이다. CGM 그래프처럼 보이는 순간
  사용자는 그 사이 값도 안다고 착각한다.
- **목표 범위는 중립적인 띠 하나**다. 안팎을 초록/빨강으로 칠하면 그건 판정이고,
  같은 이유로 비율 숫자에도 색을 입히지 않는다.
- **"건수 기준이지 시간 기준이 아니다"** 를 상시 노출하고 테스트로 고정했다.
  CGM 의 TIR 로 읽으면 임상적으로 다른 의미가 된다.
- **0건과 "평균 0" 을 구분한다.** 기록이 없으면 숫자를 아예 그리지 않고,
  기록이 없는 태그도 0 으로 채우지 않는다. 0 은 측정값이고 부재는 아니다.

**배치를 한 번 갈아엎었다.** 요약을 카드 넷으로 시작했더니 세로로 쌓이면서
그래프가 첫 화면 밖으로 밀려났다 — 숫자를 보러 온 사람이 스크롤부터 해야 하는
화면이 됐다. 카드 하나로 합쳤고, **차트 상단이 뷰포트 안에 있는지 검사하는
테스트**를 붙여 나중에 카드를 끼워 넣으면 그 테스트가 먼저 깨지게 했다.

**`GlucoseStatistics` 에 테스트가 하나도 없었다.** 표본분산(n-1) 같은 판단이
들어 있는데 아무것도 고정하고 있지 않았다. 13개 채웠고 구현은 전부 정확했다.

**위젯 테스트 함정 재발**: 태그 목록이 `ListView` 뷰포트 밖이라 요소가 만들어지지
않아 "없다"로 잘못 통과했다. W12 의 저장 버튼 건과 같은 종류다 — 긴 화면에서는
`scrollUntilVisible` / `ensureVisible` 없이 단정하지 말 것.

**실기기 확인**(2026-08-21): 서버에 26건을 넣어 pull 로 받아 그래프를 채웠다.
**여러 행 pull** 이 이때 함께 검증됐다(그전까지는 한 건뿐이었다).

**W11 잔여 과제**
- 목표 범위가 아직 기본값(70~180) 고정이다. 설정 화면에서 사용자가 정하게 해야 한다.
- 기간별 리포트(PDF/CSV)는 별도 단계다.

### G1~G8 (2026-08-21) — GLM 위임 정리 작업

W 시리즈와 **평행하게 도는 두 번째 이력**이다. 기능을 늘리는 작업이 아니라
쌓인 부스러기를 치우는 작업이라 주(W) 번호를 주지 않았다. 지시서는
[`GLM_TASKS.md`](GLM_TASKS.md), 작업별 상세는 [`reports/`](reports/) 에 있다.
**여기에는 나중에 다시 물을 만한 것만 적는다.**

| 작업 | 결과 | 코드 커밋 |
|---|---|---|
| G1 | 미사용 l10n 키 4개 삭제(`unitLabel`·`unitMgdl`·`unitMmoll`·`actionRetry`) | `cc8d02f` |
| G2 | 삭제 되돌리기에 `readingRestored` 확인 스낵바 | `01d12b8` |
| G3 | 기록 목록 타일에 메모 한 줄 | `a3e2121` |
| G4 | 해결된 applicationId TODO 삭제 | `52fd501` |
| G5 | `LICENSES.md` 의 "확인 필요" 7건 실사 | `75b6302` |
| G6 | 기록 타일·통계 요약 카드에 `Semantics` 라벨 | `cc4d32a` |
| G7 | 예외 원문 노출 제거, `readingsLoadFailed` 로 대체 | `4d62001` |
| G8 | es·pt·de·fr 4개 언어 추가 (+영어 폴백 `359a5b7`) | `44d4f74` |

**l10n 키 수가 103 → 100 으로 바뀌었다.** G1 이 4개를 지우고 G7 이
`readingsLoadFailed` 하나를 더했다. 6개 ARB 파일이 전부 100키로 일치하고,
번역본에는 `@` 메타데이터를 넣지 않는다(`app_ko.arb` 형식). 이 규약을 깨면
`flutter gen-l10n` 이 원본 메타데이터와 충돌한다.

**미지원 로케일은 영어로 떨어진다.** 언어를 6개로 늘리자 Flutter 기본 해석이
목록 첫 항목(`de`)을 집어, 일본어 사용자가 독일어 앱을 보게 됐다. `app.dart` 의
`resolveAppLocale` 이 이걸 막는다 — `test/app/locale_fallback_test.dart` 가 고정.
**지원 언어를 더 추가할 때 이 테스트를 먼저 볼 것.**

**의료 문구 21개는 기계 번역을 그대로 넣지 않았다.** 4개 언어 × 21키를 DeepL
역번역으로 교차 검증해 의미 보존 84/84, 판정어 0건을 확인했다
([reports/G8-backtranslate-input.md](reports/G8-backtranslate-input.md)).
번역에서 실제로 갈렸던 자리:
- `statsInRangeNote` 는 **"건수 vs 시간"** 대비가 살아 있어야 한다. 네 언어 모두
  CGM/MCG 를 명시해 TIR 과 구분했다.
- `editUnitWarning` 은 **"재해석"** 이지 "변환"이 아니다. 어느 언어든 convert 계열
  동사로 옮기면 뜻이 정반대가 된다.
- `statsLow`/`statsHigh` 는 최저/최고값이지 "낮음/높음"이 아니다. 판정어로 읽히는
  번역을 피해 Mínimo/Máximo·Tiefstwert/Höchstwert 등으로 옮겼다.
- `statsByTag` 의 "tag" 는 독일어 `Tag`(=날)와 충돌해 네 언어 모두 "카테고리"로 옮겼다.

**G6 은 새 l10n 키를 만들지 않았다.** 스크린리더 라벨을 기존 현지화 조각을 조합해
구성했다 — 새 키를 만들면 같은 문장이 두 벌이 되고 그만큼 의료 문구 검토 대상도
...
- **소수점 표기**(de/fr/es 권 쉼표)는 손대지 않았다. `GlucoseUnit.format()` 이
...
기록이 서버로 올라간다. 기능이 빠진 정도가 아니라 **출시 자체가 막히는 구멍**이라
여기 적어 둔다.

**적용되는 법은 EU 와 무관하게 이미 있다.** "우리는 의료기기가 아니다"는 의료기기
규제(MDR/DiGA)에는 통하지만 개인정보 법제에는 아무 영향이 없다. 앞쪽은 앱이
**무엇을 주장하는가**로 걸리고, 뒤쪽은 **어떤 데이터를 보유하는가**로 걸린다.
혈당값을 저장하는 순간 후자에 들어간다.

| 사용자 소재 | 적용 법 | 요구되는 것 |
|---|---|---|
| **한국** | 개인정보보호법(PIPA) | 민감정보 처리 동의 + **국외 이전 별도 동의** |
| EU | GDPR Art.9 | 명시적 동의 + 이전 근거(SCC 등) |
| 브라질 | LGPD | — |
| 멕시코 | LFPDPPP | — |

한국 사용자만 받아도 걸리는 이유: 서버가 `ap-southeast-1`(싱가포르)이라 모든
저장이 **국외 이전**이다. PIPA 제28조의8 의 요건 중 실무적으로 쓰는 것은
정보주체의 **별도 동의**이고, 동의를 받을 때 이전받는 자의 명칭·연락처, 이용
목적, 보유·이용 기간을 **미리 알려야** 한다. 이 항목들이 바뀌면 다시 알리고
동의를 받아야 한다.

**리전을 EU 로 옮겨도 이 의무는 사라지지 않는다.** 한국에 서버를 두지 않는 한
국외 이전인 것은 같다. 그래서 리전 선택은 규제 문제라기보다 지연시간 문제로
보는 편이 맞다(현재 싱가포르, 주 사용자가 한국이면 유리).

**해야 할 일**
- 로그인 전 동의 화면 — 민감정보 처리 동의, 국외 이전 동의를 **각각** 받는다.
  하나로 묶지 말 것. 고지 항목(이전받는 자 = Supabase, 국가 = 싱가포르, 목적,
  보유 기간)을 화면에 적는다.
- 개인정보처리방침 문서와 공개 URL. **Play 스토어 등록에 필수**다.
- Play Console 의 Data safety 양식 — 건강 데이터 수집·전송을 신고한다.
- 동의 철회와 계정 탈퇴 경로. 탈퇴 시 hard delete(W9 잔여 과제와 연결).
- 배포 국가를 정한다. EEA 를 포함할지에 따라 SCC 문서가 붙는다.

**GLM 에 넘기지 않는다.** 법적 문구이고, 동의 항목을 잘못 나누면 동의 자체가
무효가 된다.
