...
> **가중 기준(사람 결정 (가), 2026-09-24)**: 합성 프로파일 12종 균등을
> 기기 충실로 유지한다 — 그래서 **기준선과 비교할 때도 사진 가중이 아니라
> 기기 균등(band_du)을 본다.** 극성은 이미 기기 단위(by_device)다.
> 밀도·링도 density_du·ring_du 를 보고 기준으로 추가했다(collect_* 의
> with_ids 확장) — 밀도는 사후 보고 전용, 링 확률의 재튜닝은 하지 않는다.
> 사진 가중치는 촬영자 편향이다 — 대규모 단말의 사진 수가 기준선을 끌면
> 안 된다(실증: 가로형 cx p10 이 사진 가중 0.304 → 기기 균등 0.456).
