...
## 4.1 OCR 작업군 (G18·G19·G22·G23) — 예외적으로 위임한다
...
순서: **G22 → G23 → G19.** G18 은 독립이라 아무 때나.
(G15·G16·G17·G20 은 완료 → [`DONE.md`](DONE.md))

**G22 가 맨 앞인 이유**: 2026-09-04 에 **G20 이 놓친 네 번째 파일**이 드러났다
(`eval_reader.py` — 채점 경로만 EXIF 무처리). 학습 입력은 고쳐졌는데 채점만 옛
좌표계에 남아 **"CTC 완전일치 11.7%"** 라는 가짜 기준선이 나왔고, 그 위에 하루치
가설 문서가 세워졌다. 고치자 재학습 없이 **92.9%** 였다. 다섯 번째 파일이
남아 있는지부터 세는 것이 먼저다.

---

### G22 — 이미지 디코드 자리 전수 감사 (**보고가 본체**, 수정은 지정된 것만)

**상태**: 대기 · **G23·G19 보다 먼저 할 것**

2026-09-02 의 G20 이 EXIF 처리를 통일했는데 **`eval_reader.py` 를 빠뜨렸다.**
학습 입력은 표시 좌표계로 고쳐지고 **채점만 옛 좌표계에 남아**, 원본의 83.5%를
옆으로 누운 채 채점했다. 그 결과가 "CTC 완전일치 11.7% · 길이 붕괴 61%" 였고,
`exif_transpose` 한 줄을 넣자 **재학습 없이 92.9% · 2.9%** 가 됐다(`b015830`).

**이 작업은 다섯 번째 파일이 있는지 세는 것이다.** 고치는 것보다 빠짐없이 찾는
것이 목적이다.

먼저 읽을 것 (**셋 다, 시작 전에**):
- `doc/wiki/antipatterns/mixed-image-decode-conventions.md` — 디코더별 관례 실측표
- `doc/wiki/antipatterns/metric-path-not-under-test.md` — 왜 채점 경로가 빠졌나
- `doc/raw/2026-09-04.md` Case 1

할 일
1. `assets_dev/train/` 과 `tools/` 아래에서 **이미지를 여는 모든 자리**를 찾는다.
   최소한 이만큼:
   ```bash
   grep -rn "Image.open\|cv2.imread\|imdecode\|decodeImage\|decodeJpg" \
     assets_dev/ tools/ --include=*.py --include=*.dart
   ```
2. 자리마다 **표 한 줄**을 적는다 — 파일:줄 / 디코더 / EXIF 처리 여부(실측) /
   **그 이미지에 물리는 좌표가 어느 좌표계인지** / 판정.
   판정은 셋 중 하나: `어긋남` / `일치` / `무관`(좌표를 안 쓰는 자리).
3. **`어긋남` 인 것만 고친다.** 고치는 방법은 하나뿐이다 —
   `pil = ImageOps.exif_transpose(pil)` 를 `convert("RGB")` **앞에**.
   `cv2.imread` 경로는 **그대로 둘 것**(이미 EXIF 를 적용한다).
4. 고친 자리마다 **고친 뒤 숫자를 한 줄 찍어 보고서에 붙인다.** 채점 스크립트면
   재실행 결과, 캐시 빌더면 표본 크롭 몇 장을 PNG 로 떨어뜨려 눈으로 본 결과.

주의 — **여기서 틀리기 쉬운 자리**
- **"무처리일 것이다"를 코드 확인 없이 적지 말 것.** G20 지시서가 정확히 그렇게
  해서 무죄인 파일 둘을 가해자로 지목했다(`doc/raw/2026-09-02.md` Case 13).
  `package:image` 의 `decodeImage` 는 **디코드 시점에 orientation 을 굽는다** —
  손대면 오히려 이중 회전이 된다.
- **수정했다고 적기 전에 실행할 것.** 2026-09-02 에 GT 패딩 버그를 "고쳤다"고
  기록했는데 상수를 잘못 비교해 **no-op 이었고**, 그 거짓 안전 신호가 3일간
  남아 완전일치를 18.3pp 낮게 찍었다(`doc/raw/2026-09-04.md` Case 2).
  필터를 고쳤으면 **걸러진 개수를 세어** 볼 것.
- **`lib/` 를 건드리지 말 것.** 앱의 사진 불러오기 경로는 사람이 정할 일이다.
- 캐시를 다시 만들었으면 **옛 캐시를 지울 것.** 방향이 섞인다.

완료 기준: 감사 표에 모든 디코드 자리가 있고 각각 판정이 붙는다. `어긋남` 이
0건으로 남는다. 고친 자리마다 **고친 뒤 숫자**가 보고서에 있다.
`어긋남` 이 0건이면 "0건"이라고 적는다 — 억지로 찾아내지 말 것.

**보고서**: `docs/reports/G22-decode-site-audit.md`

---

### G23 — CTC 오류 104건을 성격별로 분류 (**분류만** 한다, 모델 무수정)

**상태**: 대기 · **G22 뒤에 할 것**

계측 버그를 고치자 CTC 리더는 캐시 holdout 1,122장에서 **92.9%** 를 낸다.
남은 오류가 진짜 과제인데, **지금은 그게 몇 종류인지 아무도 모른다.**

배경: [`assets_dev/train/claude_consult_ocr_response.md`](../assets_dev/train/claude_consult_ocr_response.md) §4.
**`claude_consult_ocr.md` 는 무효 문서다** — 맨 위 경고를 읽고, 그 안의 가설
(H1 워프 압착 · H2 도메인 불일치)을 실행하지 말 것.

할 일
1. `diag_timestep.py` 를 참고해(재학습 없이 기존 체크포인트로 채점한다)
   **오답 전량**을 아래 규칙으로 분류한다. **이 규칙을 그대로 쓸 것 — 정하지 말 것:**

   | 분류 | 규칙 | 위험도 |
   |---|---|---|
   | **한 글자 오독** | 자릿수 같고 편집거리 1 (`98`→`92`, `115`→`119`) | **위험** |
   | 여러 글자 오독 | 자릿수 같고 편집거리 2 이상 | 위험 |
   | 자릿수 증가 | 예측이 GT 보다 김 (`122`→`1122`) | 안전 |
   | 자릿수 붕괴 | 예측이 GT 보다 짧음 (`119`→`9`) | 안전 |
   | **전부 blank** | 40 타임스텝 중 비-blank 가 **2칸 이하** | 조사 대상 |

2. 분류별 **건수 표**를 낸다. 각 분류에서 **예시 5건**을 id 와 함께 적는다.
3. `전부 blank` 로 분류된 장은 **워프 결과를 PNG 로 떨어뜨려 눈으로 본다.**
   화면이 아닌 곳을 잡았는지(쓰레기 rect), 극단 조건인지 세어서 적는다.
   **몇 장을 봤고 그중 몇 장이 쓰레기 rect 였는지** 숫자로 적을 것.
4. `한 글자 오독` 에서 **어느 숫자 쌍이 자주 헷갈리는지** 혼동 표를 낸다
   (`8`↔`9`, `0`↔`8` 같은 쌍의 건수).

주의 — **여기서 틀리기 쉬운 자리**
- **모델·워프·학습 코드를 한 줄도 고치지 말 것.** 이 작업의 산출물은 표다.
  고치고 싶은 것이 보이면 **고치지 말고 보고서에 적는다.**
- **위험도 칸을 재해석하지 말 것.** 자릿수가 무너진 오류가 "더 나쁜" 것이
  아니다 — 값이 범위를 벗어나 검증기에 걸리므로 오히려 안전하다. 위험한 것은
  `98`→`92` 처럼 **그럴듯해서 안전망을 통과하는** 오독이다.
- **정확도를 올리려 하지 말 것.** 임계값·기하·에폭 수를 만지면 이 표가
  무엇을 재고 있는지 알 수 없게 된다.
- 재학습하지 말 것. 기존 `reader_model`(2026-09-03 23:21) 그대로 쓴다.
  재학습이 필요해 보이면 보고서에 적고 멈출 것.

완료 기준: 오답 전량이 5개 분류로 나뉘고 건수·예시가 있다. `전부 blank` 장의
눈검 결과가 숫자로 있다. 혼동 표가 있다. **코드 변경 0줄**(진단 스크립트 추가는 허용).

**보고서**: `docs/reports/G23-ctc-error-taxonomy.md`
