...
- **팔 1 (안전)**: **세로형만** 세로 `[0.00, 0.72]` 를 남긴다. 가로형은 손대지
  않는다.
- **팔 2 (조건부)**: 세로형은 세로 `[0.00, 0.72]`, **가로형은 가로
  `[0.00, 0.85]`** 를 남긴다.

형태 판정은 **원본 GM 박스의 `w/h > 1.2`** 를 가로형으로 한다.

**가로형 기준이 0.72 가 아니라 0.85 인 이유**: GM 검출기가 가로형에서 사람
라벨의 오른쪽을 **27.5% 덜 덮는다**(실측, n=42). 이미 오른쪽이 잘려 있는
박스를 0.72 에서 또 자르면 숫자가 잘린다. 이 값을 GLM 이 바꾸지 말 것.
...
- 캐시 `data_cache_v2_bandcrop_a.npz` / `..._b.npz`
- 모델 `reader_model_bandcrop_a` / `..._b`
- 예측 `reader_preds_bandcrop_a.json` / `..._b.json`
...
- **가로형/세로형 층별 정확도를 따로** 적을 것. 가로형은 51장(전체의 2.0%)
  뿐이라 전체 평균에 흔적을 안 남긴다. 층을 안 나누면 팔 2 의 효과가
  보이지 않는다. 가로형 id 는 `_diag/wide_all/wide_ids.json` 에 있다.
