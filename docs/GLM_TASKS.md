...
**왜 하는가.** G29 가 letterbox 를 기각하면서 축을 알려줬다. **타임스텝은
가로에서만 나온다** — Reshape 가 세로를 특징 차원으로 접기 때문이다. letterbox
는 내용 폭을 320 → 125px 로 줄여 유효 타임스텝을 40 → 15.6 으로 깎았고(숫자당
13.3 → 5.2), CTC 가 blank 를 찍을 여유를 잃어 위험군이 0.98% → 3.48% 로 뛰었다.

**크롭은 축소가 아니다.** 숫자가 없는 부분을 잘라내고 남은 것을 320×160 으로
늘리면 숫자가 **커진다.** 방향이 letterbox 와 정반대다.

밴드 라벨 86장(가로형 51장 전수 포함)으로 숫자줄의 위치를 쟀다. 사람 LCD
라벨 안에서의 상대 위치이며, 회전 1장(`glucose_batch1/1564`)은 제외했다:

| | 가로형 n=46 | 세로형 n=34 |
|---|---|---|
| 밴드 높이비 중앙 | **0.798** | 0.485 |
| 밴드 폭비 중앙 | 0.491 | **0.795** |
| 아랫변 p95 | 0.952 | **0.658** |
| 우변 p95 | **0.643** | 0.948 |
| 중심 흩어짐(p5~p95) | 0.064 | 0.062 |

**두 형태가 전치 관계다.** 세로형은 숫자가 위쪽에서 높이의 절반만 쓰고 아래가
날짜·아이콘이다. 가로형은 숫자가 왼쪽에서 폭의 절반만 쓰고 오른쪽이
날짜·아이콘이다. 어느 쪽이든 **숫자가 없는 축의 뒤쪽이 남는다.**

흩어짐이 0.06 대로 작아서 **검출기 없이 고정 비율로 자를 수 있다.**

**이 작업은 채택하지 않는다.** 수치를 내고 멈춘다.

---

#### 한 팔만 돌린다 — 세로형 전용

**세로형만** 세로 `[0.00, 0.72]` 를 남긴다. **가로형은 손대지 않는다.**

형태 판정은 **원본 GM 박스의 `w/h > 1.2`** 를 가로형으로 한다.

**가로형을 크롭하지 않는 이유** (2026-09-08 실측, 밴드 라벨 86장):
비율의 기준이 되는 GM 박스가 가로형에서는 밴드를 담고 있지 않다.

| GM 박스 기준 밴드 위치 | 가로형 n=46 | 세로형 n=21 |
|---|---|---|
| 아랫변 p5~p95 폭 | 0.436 (최대 +14.4) | **0.100** |
| 좌변 p5~p95 폭 | **1.309** (p5 −0.944) | 0.310 |
| 밴드가 박스 밖으로 나간 장 | 왼 14 · 아래 6 / 46 | **0 / 21** |

가로형 46장 중 14장에서 밴드가 박스 **왼쪽 바깥**에 있다. 박스가 숫자를
담고 있지 않은데 그 박스의 몇 %를 자르는 규칙은 의미가 없다.
**가로형은 크롭 문제가 아니라 검출 문제다** — 별도 작업으로 뺀다.

**위쪽은 자르지 않는다.** 인접 아이콘(배터리 등)을 빼려고 `[0.05, …]` 를
쓰고 싶어지지만, 세로형 밴드 윗변 p5 가 **0.077** 이라 5%의 장에서 숫자
윗획이 나간다. `0.00` 을 바꾸지 말 것.

**비율의 기준은 `BOX_MARGIN` 을 적용하기 _전_ 의 원본 GM 박스다.** 위 측정이
그 좌표계에서 나왔다. 순서는 ①원본 박스에서 구간을 잘라 새 박스를 만들고
②거기에 `BOX_MARGIN` 을 적용한다. 뒤집으면 크롭이 밀리고, 그 밀림이
"효과 없다" 로 보인다.

**세로 지터를 넓힌다.** 크롭이 조금 어긋나도 견디게 해야 한다.
`ctc_reader_v2.py` 의 `RandomTranslation` 세로 성분을 0.08 → **0.15**.
가로 성분은 건드리지 않는다. 그 밖의 하이퍼파라미터는 하나도 바꾸지 않는다.

#### 산출과 보고

- 캐시 `data_cache_v2_bandcrop.npz`
- 모델 `reader_model_bandcrop`
- 예측 `reader_preds_bandcrop.json`
  (아틀라스 `--compare-preds` 가 읽는다)
- **`reader_model` 과 `data_cache_v2.npz` 를 덮어쓰지 말 것.**

보고할 것:
- 완전일치 % / **위험군(자릿수 보존 오독) %** / blank / 안전 실패
- McNemar 정확 이항 p값 (G29 기준선 97.06% / 위험군 0.98% 와 짝지은 비교)
- **새로 틀린 장의 id 전체 목록** — 채택 판단이 여기서 갈린다
- **가로형/세로형 층별 정확도를 따로** 적을 것. 가로형은 크롭하지 않으므로
  기준선과 같아야 한다 — 달라지면 형태 판정이나 좌표계가 틀린 것이다.
  가로형 id 는 `_diag/wide_all/wide_ids.json` 에 있다.
...
**건드리지 말 것**
- `reader_model/`, `data_cache_v2.npz`, `reader_preds.json` — 읽기만 한다.
- `band_boxes.jsonl`, `band_rotation.jsonl`, `screen_boxes.jsonl`,
  `wide_band_queue.json` — 읽기만 한다.
- `BOX_MARGIN` 값, 모델 구조, 에폭 수, 그 밖의 하이퍼파라미터.
  (`RandomTranslation` 세로 성분만 예외)
- `make_failure_atlas.py`, `webtool.py` — 다른 작업 영역이다.
- `lib/` 아래 전부. 새 파이썬 패키지 설치 금지.

**틀리기 쉬운 자리**
- **좌표계 순서**(위에 명시).
- **학습과 추론이 같은 함수를 써야 한다.** `eval_reader.py` 의 크롭에도 같은
  구간이 적용돼야 한다. 기하는 `frame_crop` 한 곳에 있다.
- 합성 이미지에는 **적용하지 않는다.** 합성은 이미 숫자 중심으로 생성돼
  구간 개념이 없다. 실사진 경로에만. (판단하지 말고 그대로 따를 것)
- `glucose_batch1/1564` 는 가로형 기기를 눕혀 찍은 장이라 밴드가 세로로 길다.
  `band_rotation.jsonl` 에 기록돼 있다. **위 비율을 적용하면 숫자가 잘린다** —
  이 id 는 크롭에서 제외하고 원본 박스를 그대로 쓸 것.
- hold-out 분할 시드가 기준선과 같은지 확인하라. 다르면 비교가 무효다.
- TTA 시드는 G29 가 `zlib.crc32` 로 고정했다. 되돌리지 말 것.
- 실행 환경은 conda `sugartrain`.

**검증**: `flutter analyze` / `flutter test` 를 커밋 전에 돌려 통과 확인.

**브랜치**: `glm/G30-band-crop`
**보고서**: `docs/reports/G30-band-crop.md`

### G29 — TTA 시드 고정 + letterbox 워프 A/B

**상태**: 대기 · **최우선** · G28 과 독립

**왜 하는가.** 워프가 LCD 종횡비와 무관하게 무조건 `320×160`(2:1) 로 늘린다.
LCD 박스의 종횡비 중앙은 0.78(세로가 김)이라 **가로가 세로 대비 평균 2.55배
늘어나고, 그 배율이 기기마다 1.0 ~ 3.3 으로 흩어진다**(hold-out 실측). 같은
숫자가 기기에 따라 전혀 다른 모양으로 리더에 들어간다.

오답이 그 꼬리에 몰려 있다. **검출이 정상인데 틀린 17건 중 8건(47%)이 정답
분포의 p10~p90(2.10~2.90) 밖**이다. 실패 아틀라스가 "이미지 성질로는 변별 축을
못 찾았다"고 적은 그 17건인데, 축이 이미지가 아니라 **워프 파라미터**에 있었다.

비율을 유지하고 여백을 채우면(letterbox) 이 편차가 사라진다. 대신 해상도를
잃는다(세로로 긴 LCD 는 320px 중 약 125px 만 내용). **어느 쪽이 이기는지는
재 봐야 안다 — 그래서 A/B 다.**

**이 작업은 채택하지 않는다.** 새 모델을 만들고 수치를 내는 데까지다.
`reader_model` 을 교체하는 결정은 사람이 한다.

---

#### A. TTA 시드를 재현 가능하게 (**먼저 한다**)

`eval_reader.py` 의 `np.random.RandomState(TTA_SEED + (hash(cid) & 0xFFFF))` 에서
파이썬 `hash()` 는 **프로세스마다 솔트가 달라** 같은 이미지가 실행마다 다른
변형을 받는다. 지금 상태로는 A/B 수치를 믿을 수 없다.

- `hash(cid)` → `zlib.crc32(cid.encode("utf-8"))` 로 바꾼다. 이것만 바꾼다.
- **바꾼 뒤 현재 `reader_model` 로 평가를 다시 돌려 기준선을 새로 잡는다.**
  기존 96.88% / 위험군 1.07% 는 불안정한 시드로 잰 값이라 비교 대상이 못 된다.
  새 기준선을 보고서에 적고, 그 값을 B 의 대조군으로 쓴다.

#### B. letterbox 워프

**공통 함수 하나를 만들어 학습·추론 양쪽이 그것을 쓴다.** 두 곳에 따로 구현하면
프레이밍이 갈리고, 그 불일치 자체가 성능 저하로 나타나 원인을 오독하게 된다
(`antipatterns/duplicated-geometry-implementation`).

- 원본 박스의 종횡비를 유지한 채 `320×160` 안에 최대 크기로 넣고, 남는 변을
  **상수 128(중간 회색)** 로 채운다. 가운데 정렬.
- **채움 값을 바꾸지 말 것.** 0(검정)은 백라이트 화면의 어두운 배경과, 255는
  일반 화면의 밝은 배경과 섞인다. 128 은 둘 다와 구별된다.
- 적용 대상: `build_cache_v2.py` 의 실사진 크롭 **그리고 합성 이미지**
  (지금 `cv2.resize` 로 늘리고 있다 — 학습셋 안에서 규약이 갈리면 안 된다),
  `eval_reader.py` 의 추론 크롭.
- `BOX_MARGIN` 은 **건드리지 않는다.** 한 번에 한 변수만 바꾼다.

#### C. 재학습과 측정

- `ctc_reader_v2.py` 를 **하이퍼파라미터·구조·에폭 수를 하나도 바꾸지 않고**
  돌린다. 바뀐 것은 입력 프레이밍뿐이어야 한다.
- 산출 경로를 `reader_model_letterbox` 로 한다. **`reader_model` 을 덮어쓰지
  말 것.** (2026-09-04 에 합성 라벨을 덮어써 복구에 시간을 쓴 적이 있다.)
- 캐시도 `data_cache_v2_letterbox.npz` 로 따로 낸다. 기존 캐시를 덮지 않는다.
- 같은 hold-out 으로 두 모델을 평가하고 다음을 보고한다:
  - 완전일치 %
  - **위험군(자릿수 보존 오독) %** — 이 앱에서 가장 중요한 수치다.
    자릿수가 무너지거나 침묵하는 실패는 안전하지만, 값이 그럴듯한 오독은
    범위 검증기도 프레임 합의도 통과한다.
  - McNemar 검정 p값 (짝지은 비교 — 같은 장을 두 모델이 어떻게 갈랐는가)
  - 늘림 배율 층별 정확도: `<2.10` / `2.10~2.90` / `>2.90`
    (기준선에서 오답이 몰려 있던 층이 실제로 좋아졌는지가 이 실험의 핵심 질문이다)

**판정을 GLM 이 하지 말 것.** 위 수치를 표로 내고 멈춘다. 채택 여부는 사람이
정한다.

---

**건드리지 말 것**
- `reader_model/`, `data_cache_v2.npz` — 읽기만 한다. 새 이름으로 낸다.
- `BOX_MARGIN`, 모델 구조, 학습 하이퍼파라미터, 에폭 수.
- `lib/` 아래 전부. `webtool.py`. 기존 `diag_*.py`.
- 새 파이썬 패키지 설치 금지.

**틀리기 쉬운 자리**
- **학습과 추론이 같은 함수를 써야 한다.** `eval_reader.py` 에 letterbox 를
  안 넣고 학습만 바꾸면 성능이 떨어지는데, 그 원인이 letterbox 로 보인다.
- 합성 이미지를 빠뜨리기 쉽다. `build_cache_v2.py` 의 합성 경로는 별도
  `cv2.resize` 라 실사진 경로만 고치면 학습셋 안에 두 규약이 섞인다.
- EXIF: 좌표 정본은 `cv2.imread`(EXIF 적용) 기준 표시 좌표계다. PIL 은 EXIF 를
  무시하므로 `ImageOps.exif_transpose` 를 빼면 안 된다.
- hold-out 정의는 캐시의 `real_train_ids` 를 뺀 것이다. `labeled.jsonl` 로
  정의하지 말 것. **letterbox 캐시를 새로 만들면 분할 시드가 같은지 확인하라**
  — 분할이 달라지면 두 모델이 다른 hold-out 을 보게 되어 비교가 무효다.
- 실행 환경은 conda `sugartrain`. 없이 돌리면 `No module named 'cv2'`.

**검증**: 파이썬만 건드리므로 `flutter analyze` / `flutter test` 는 영향받지
않지만 커밋 전에 둘 다 돌려 통과를 확인할 것 — `lib/` 우발 변경을 잡아 준다.

**브랜치**: `glm/G29-letterbox-warp`
**보고서**: `docs/reports/G29-letterbox-warp.md`
