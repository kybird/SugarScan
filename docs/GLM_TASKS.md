...
### G30 — 세로 크롭 A/B (숫자줄 위쪽 고정 비율만 남긴다)

**상태**: 대기 · **최우선** · G29 완료 후

**왜 하는가.** G29 가 letterbox 를 기각하면서 축을 하나 알려줬다. **타임스텝은
가로에서만 나온다** — 컨볼루션 뒤 Reshape 가 세로를 특징 차원으로 접기 때문에
세로 픽셀은 타임스텝 예산에 한 칸도 기여하지 않는다. letterbox 는 내용 폭을
320 → 125px 로 줄여 유효 타임스텝을 40 → 15.6 으로 깎았고(숫자당 13.3 → 5.2),
CTC 가 숫자 사이 blank 를 찍을 여유를 잃어 위험군이 0.98% → 3.48% 로 뛰었다.

**세로는 정반대다.** 숫자줄은 크롭 폭의 81% 를 쓰지만 높이는 **49.8%** 만 쓴다.
나머지는 날짜·시각·아이콘이다. 세로를 잘라내면 **타임스텝을 한 칸도 잃지 않고**
숫자의 세로 해상도만 올라간다.

그리고 자를 위치를 검출할 필요가 없다. GM 박스 안에서 숫자줄의 세로 위치가
기종을 넘어 일정하다(밴드 라벨 21장, 박스 상단 0 · 하단 1 기준):

| | p5 | 중앙 | p95 |
|---|---|---|---|
| 윗변 | 0.077 | 0.101 | 0.176 |
| 아랫변 | 0.550 | 0.599 | 0.651 |
| 중심 | 0.320 | 0.356 | **0.389** (흩어짐 0.069) |

21장 전부를 덮는 최소 구간이 [0.071, 0.658] 이다. **박스의 고정 비율로 자르면
된다.**

**단, n=21 이고 기종 편중 여부를 모른다.** 숫자를 화면 아래쪽에 띄우는 기종이
있으면 그 기종은 통째로 못 읽게 된다 — 지금보다 나쁜 실패다. 그 검증을 라벨로
하지 않고 **hold-out 1,122장 A/B 로** 한다. 기종을 전부 포함하므로 더 강한
검증이고, 깨지는 기종이 있으면 "새로 틀림" 으로 드러난다.
...
같은 학습 설정으로 두 번 학습하고, G29 가 세운 기준선과 각각 비교한다.

- **팔 1 (안전)**: 박스 세로의 **[0.00, 0.75]** 만 남긴다
- **팔 2 (공격)**: 박스 세로의 **[0.03, 0.68]** 만 남긴다

관측 아랫변 p95 가 0.651 이므로 팔 1 은 여유가 넉넉하고 팔 2 는 빠듯하다.
둘을 비교하면 "도움이 되는가" 와 "얼마나 밀어붙일 수 있는가" 가 한 번에 나온다.

**비율의 기준은 `BOX_MARGIN` 을 적용하기 _전_ 의 원본 GM 박스다.** 위 측정이
그 좌표계에서 나왔다. 마진 적용 후 박스에 같은 비율을 쓰면 크롭이 위아래로
밀린다. 순서는 ①원본 박스에서 세로 구간을 잘라 새 박스를 만들고 ②거기에
`BOX_MARGIN` 을 적용한다.

**세로 지터를 넓힌다.** 크롭이 조금 어긋나도 견디게 해야 실제 검출 오차에서
무너지지 않는다. `ctc_reader_v2.py` 의 `RandomTranslation` 세로 성분을
0.08 → **0.15** 로 올린다. 가로 성분과 다른 값은 건드리지 않는다.
**이것 말고 다른 하이퍼파라미터는 하나도 바꾸지 않는다.**

#### 산출과 보고

- 캐시 `data_cache_v2_vcrop75.npz` / `..._vcrop68.npz`
- 모델 `reader_model_vcrop75` / `reader_model_vcrop68`
- **`reader_model` 과 `data_cache_v2.npz` 를 덮어쓰지 말 것.**
- 예측 `reader_preds_vcrop75.json` / `..._vcrop68.json`
  (아틀라스 `--compare-preds` 가 읽는다)

각 팔에 대해 보고할 것:
- 완전일치 % / **위험군(자릿수 보존 오독) %** / blank / 안전 실패
- McNemar 정확 이항 p값 (기준선과 짝지은 비교)
- **새로 틀린 장의 id 전체 목록** — 채택 판단이 여기서 갈린다. 한 기종이
  통째로 무너지는지 보려면 개별 id 가 있어야 한다
- 늘림 배율 층별(<2.10 / 2.10~2.90 / >2.90) 정확도

**판정을 GLM 이 하지 말 것.** 표를 내고 멈춘다.
...
- `BOX_MARGIN` 값, 모델 구조, 에폭 수, 그 밖의 하이퍼파라미터.
  (`RandomTranslation` 세로 성분만 예외 — 위에 명시)
- G29 가 남긴 letterbox 산물 — 지우지 말고 그대로 둔다.
- `make_failure_atlas.py` — 별도 작업이 돌고 있다.
- `lib/` 아래 전부. `webtool.py`.
- 새 파이썬 패키지 설치 금지.

**틀리기 쉬운 자리**
- **좌표계 순서**(위에 명시). 이걸 뒤집으면 크롭이 밀리고, 그 밀림이
  "세로 크롭은 효과 없다" 로 보인다.
- **학습과 추론이 같은 함수를 써야 한다.** `eval_reader.py` 의 크롭에도 같은
  세로 구간이 적용돼야 한다. G29 병합 후 기하는 `frame_crop` 한 곳에 있다.
- 합성 이미지에는 **적용하지 않는다.** 합성은 LCD 화면이 아니라 이미 숫자
  중심으로 생성된 것이라 세로 구간 개념이 없다. 실사진 경로에만 적용한다.
  (판단하지 말고 그대로 따를 것)
...
**브랜치**: `glm/G30-vertical-crop`
**보고서**: `docs/reports/G30-vertical-crop.md`
