...
순서: **G31 먼저.** G32·G18·G19 는 독립이라 아무 때나.
(G15~G17·G20·G22~G30 은 완료 → [`DONE.md`](DONE.md))
...
**왜 다시 하는가.** G30 이 낸 87.79% 는 프레이밍의 성적이 아니다. 내가 쓴
G30 지시서가 "에폭 수를 건드리지 말 것"이라고 적었는데, 그 말이 가리킨
스크립트 기본값(파인튜닝 12에폭)이 **기준선 모델이 실제로 받은 예산이 아니었다.**

`assets_dev/train/training_log.csv` 는 `reader_model/` 과 mtime 이 같은
현 기준선의 로그이고, 파인튜닝 구간이 재시작 7회로 쌓여 있다. 마지막 런이
**60에폭 · 최종 손실 0.1238** 이다(직전 두 런도 60에폭 · 0.135 · 0.102).
크롭 팔은 **12에폭 · 최종 손실 0.96** 에서 멈췄고 보고서에도 하강 중이라고
적혀 있다. 손실이 8배 차이 나는 두 모델을 비교한 것이라 완전일치 격차
97.06% → 87.79% 를 프레이밍 탓으로 읽을 수 없다.

같은 구멍이 G29(letterbox, 12에폭 · 0.64)에도 있었다. G29 는 타임스텝이라는
학습 예산과 무관한 기전이 따로 있어 기각을 유지하지만, **크롭은 그 기전으로는
오히려 유리해야 하는 방향**이다(세로만 자르므로 가로 해상도 = 타임스텝 40 이
그대로다). 유리해야 할 팔이 letterbox 보다도 못 맞춘 것 자체가 "덜 학습됐다"
쪽 신호다. 그래서 한 번 더 잰다.
...
`assets_dev/train/g30_ft_nojit.py`(크롭만 팔 드라이버, 세로 지터 0.08)를
**파인튜닝 60에폭으로** 다시 돌린다. 그 외 레시피는 G30 그대로다.

- 사전학습은 **다시 하지 않는다.** G30 이 만든 bandcrop 사전학습 best 를
  그대로 출발점으로 쓴다(조기중단이 스스로 멈춘 단계라 예산 불일치가 없다).
  예산이 어긋난 것은 고정 길이인 파인튜닝뿐이다.
- **세로 지터는 0.08 이다. 0.15 를 쓰지 말 것.** G30 이 0.15 단독으로
  학습이 붕괴함을 차분 실험으로 확정했다(손실 5.40 에서 동결, 모드 collapse).
  이건 재확인할 필요가 없다.
- 산물 이름은 새로 뗀다: `reader_model_bandcrop_ty008_e60` ·
  `reader_preds_bandcrop_ty008_e60.json` · `training_log_bandcrop_ty008_e60.csv` ·
  `checkpoints_v2_bandcrop_ty008_e60/`. G30 산물을 덮어쓰지 말 것 —
  12에폭 지점이 이 비교의 중간 관측값이라 남아 있어야 한다.
- 캐시는 G30 이 만든 `data_cache_v2_bandcrop.npz` 를 **그대로 재사용한다.**
  다시 만들지 말 것(분할 시드가 같아도 재생성은 위험만 늘린다).

#### 보고할 것

G30 보고서와 **같은 표**를 낸다 — `g30_ab_report.py` 를 그대로 쓴다.

- 완전일치 % / 위험군(자릿수 보존 오독) % / blank / 안전 실패
- McNemar 정확 이항 p값 (기준선 97.06% / 위험군 0.98% 와 짝지은 비교)
- 새로 틀린 장의 id 전체 목록
- 가로형/세로형 층별 정확도
- **파인튜닝 손실 곡선 60에폭 전체**와, 그 안에서 12·30·60에폭 지점의 손실.
  기준선 마지막 런(0.1238)과 나란히 적을 것 — 예산이 맞았는지를 사람이
  확인하는 자리다. 60에폭에서도 손실이 하강 중이면 **그렇다고 적고 멈춘다.**
  에폭을 더 늘리지 말 것.

**판정을 GLM 이 하지 말 것.** 채택/기각은 사람이 한다.

---

**건드리지 말 것**
- `reader_model/`, `data_cache_v2.npz`, `reader_preds.json`, `training_log.csv`,
  `checkpoints_v2/` — 읽기만 한다.
- G30 산물 전부(`reader_model_bandcrop*`, `reader_preds_bandcrop*.json`,
  `training_log_bandcrop*.csv`) — 읽기만 한다. 새 이름으로 낸다.
- `band_crop_box` 의 구간 `[0.00, 0.72]`, 형태 판정 규칙 `w/h > 1.2`,
  `BOX_MARGIN`, 모델 구조, 그 밖의 하이퍼파라미터 — G30 과 **한 글자도**
  달라지면 이번 비교도 무효가 된다. 이 작업에서 바뀌는 값은 에폭 수 하나뿐이다.
- `lib/` 아래 전부. 새 파이썬 패키지 설치 금지.

**틀리기 쉬운 자리**
- **바뀌는 값이 에폭 하나뿐임을 실행 전에 확인하라.** G30 의 드라이버를
  복사해 고칠 때 다른 기본값이 딸려 오면 이번 실험도 못 쓰게 된다.
- hold-out 분할이 G30·기준선과 **순서까지** 같아야 한다.
  `g30_check_cache.py` 가 이미 검사한다 — 다시 돌려 확인할 것.
- TTA 시드는 `zlib.crc32`(43789ad). 되돌리지 말 것.
- 실행 환경은 conda `sugartrain`.

**검증**: `flutter analyze` / `flutter test` 를 커밋 전에 돌려 통과 확인.

**브랜치**: `glm/G31-bandcrop-e60`
**보고서**: `docs/reports/G31-bandcrop-epoch-matched.md`

### G32 — 가로형 GM 검출 실패 13장 (**진단만** 한다)

**상태**: 대기 · 독립

G30 이 부수로 찾아냈다. 사람이 가로형으로 라벨한 `_diag/wide_all/wide_ids.json`
47장(풀 내) 중 **13장이 형태 판정 규칙(원본 GM 박스 `w/h > 1.2`)에서 세로형으로
떨어진다.** 박스의 `w/h` 가 0.56~1.10 이다 — 가로 화면인데 검출 박스가 세로로
길다는 뜻이라, GM 검출기가 화면을 "접었다".

이건 크롭 규칙의 문제가 아니라 **검출 문제**다(G30 지시서가 이미 가로형을
크롭 대상에서 뺀 것과 같은 이유). 가로형은 기준선 모델에서도 21장 중 12장
(57.1%)밖에 못 맞히는 가장 약한 층이다.

할 일 — **고치지 말고 재라.** 13장 각각에 대해:
- 원본 GM 박스와 사람 밴드 라벨(`band_boxes.jsonl`)을 한 장에 겹쳐 그린 몽타주
- 박스의 `w/h`, 밴드가 박스 밖으로 나간 방향과 양(밴드 라벨 기준 비율)
- 이 13장이 홀드아웃/학습 중 어디에 있는지

`glucose_batch1/1565` 는 따로 적을 것. `diag_wide_gm.py` 가 1564 와 함께
회전 촬영으로 문서화했는데 `band_rotation.jsonl` 에는 1564 만 있다.
**`band_rotation.jsonl` 을 고치지 말 것** — 그 파일이 캐시 생성의 입력이라
한 줄만 늘려도 분할과 프레이밍이 조용히 움직인다. 사실만 보고한다.

**건드리지 말 것**: `band_boxes.jsonl` · `band_rotation.jsonl` ·
`screen_boxes.jsonl` · `wide_ids.json` · 검출기 코드 · `lib/`. 전부 읽기만 한다.

**브랜치**: `glm/G32-wide-gm-diag`
**보고서**: `docs/reports/G32-wide-gm-failures.md`
