# 기기 편중 완화 A/B — 두 팔 다 개선 없음, 균등화 팔은 유의하게 악화 (판정 보류 — 아래 정정 참조)
...
> **정정 (2026-09-11, 검토 후 추가) — 판정을 '기각'에서 '미정'으로 낮춘다.**
>
> **두 팔 어느 쪽도 카드의 가설을 깨끗이 시험하지 않았다.** 가설은 "기기
> 편중을 줄이면 미학습 기기 성적이 오르는가"인데, B 팔은 총 학습 장수를
> 55% 줄였고(보고서도 교란으로 명시), C 팔은 행수를 유지했지만 **고유
> 사진을 1,356 -> 783 장으로 줄였다.** C 팔이 B 팔의 교란을 분리한다는
> 본문 서술은 성립하지 않는다 — C 팔도 큰 기기 사진을 버렸기 때문이다.
> 관측된 것은 "고유 다양성을 줄이면 나빠진다"이고, 그것은 이미 알던 사실이다.
>
> **보고서에 없던 교란이 하나 더 있다.** 두 팔 모두 남길 사진을
> `sorted(ids)[:quota]` 로 골랐는데(build_device_balance_split.py:63,78) id 가
> 촬영 연번이라 이 prefix 는 사실상 **연속 촬영 한두 세션**이다(예:
> SD CHECK GOLD2 의 앞 28장이 1203~1729 구간). 장수만 준 것이 아니라 장면
> 다양성이 통째로 무너진 표본이었다.
>
> 따라서 **"재배분으로는 못 살린다 / 더 모아야 한다"는 결론을 지금 인용하지
> 말 것.** 고유 1,356장을 한 장도 버리지 않는 가중 샘플링 팔(칸반 카드
> 「기기 편중 A/B 재실험 — 고유 사진 전량 유지」)의 결과가 나온 뒤에 판정한다.
>
> 살아 있는 것: 짝비교 절차와 McNemar 구현(device_ab_report.py:57, 정확
> 이항 — 검산 통과), 세 팔이 같은 홀드아웃 배열을 공유한 설계, C 팔이
> 유의하게 나빴다는 사실 자체.
