...
> **정정 (2026-09-11, 검토 후 추가)**
>
> 1. **-1.15pp 와 +0.95pp 는 통계적으로 유의하지 않다.** 두 비율 검정 결과
>    완전일치 p=0.139(차이 95% CI [-2.70, +0.39]pp), 위험 p=0.090(CI
>    [-0.17, +2.06]pp). 아래 본문이 이 델타를 확정 서술로 쓰고 있으나,
>    **인용할 값은 평균 델타가 아니다.**
> 2. **인용할 값은 기기별 편차다.** n>=20 인 15종의 완전일치 분포가
>    76.5 / 79.8 / 91.5 / 94.2 / 95.0 / 96.4 / 96.7 x3 / 98.7 / 98.8 / 100 x4 로
>    갈린다. "미학습 기기에서 평균 1pp 잃는다"가 아니라 **"낯선 기기 중
>    일부는 76~80% 로 무너진다"**가 이 측정의 발견이다. 홀드아웃 24종을 어떻게
>    뽑느냐에 따라 평균은 크게 흔들린다(본문 '한계' 절과 같은 취지).
> 3. **형제 기기 의심은 대체로 기각됐다.** train 에 색·버튼만 다른 같은 기종이
>    있는 4종(Performa 은색, Performa Nano 파랑, CareTouch 일반LCD, Active
>    원형베젤 — 144장)을 홀드아웃에서 빼고 재집계하면 나머지 20종 994장에서
>    **완전일치 95.37% / 위험 2.41% / 거절 7.3%** 로, 전체 24종 수치와 거의
>    같다. 형제 효과는 작다. 잔여 확인은 칸반 카드
>    「홀드아웃 형제쌍 4종 패널 대조 몽타주」가 맡는다.
> 4. **학습 장수가 같지 않다.** 세션 무결 기준선의 real_train 은 1,172장,
>    이 측정은 1,356장이다(+184). 분할 방식 외에 학습 장수도 움직였으므로
>    두 수치의 차이를 '기기 단절 효과'로만 읽지 말 것.
